...
echo "
   ...
   <table border=0 cellpadding=1 cellspacing=1>
    <tr bgcolor=lightgreen>
     <th><font size=-1>Location</th><th colspan=2><font size=-1>Percent<br>participation</th><th><font color=gold size=-1>High</font> <font size=-1 color=green>Total</font><font size=-1> Points</th><th><font size=-2>Individual High | Location Total<br>Points</font></th><th><font size=-1>Average<br>points<br>earned</th>
    </tr>
";
...
//   open file, read pointsw and pointso
    ...
    $width_pct = $percent;
    if ($players > 0) { $avgptsearned = $sum / $players; }
    if ($employees > 20) {$color = 'lightblue';} else {$color= 'pink';}

    echo
"    <tr>
      <th align=left bgcolor='$color'><font size=-1><a href=\"location.php?loc=$j\">$location</a></th>
      <td align=right title=\"$players / $employees\"><font size=-1>$percent</td>
      ...
      <td title=\"$sum / $players\"><font size=-2>$avgptsearned</font></td>
     </tr>
";
    $i++;
}
