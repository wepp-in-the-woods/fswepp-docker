...
#    print "$publication -- $publ\n";
     while (&get_entry) {		# read until there ain't no more; return with $authors, $title, $citation, $keyword...
#      print "$publication -- $publ\n";
...
# strip <a> and </a> from $citationt into $citationt_strip
         ...
         substr($citationt_strip,$loc_closebarea,4)='' if ($loc_closebarea > -1);
         substr($citationt_strip,$loc_barea,3)='' if ($loc_barea > -1);
...
if ($links =~ /.pdf/) {print '<img src="/images/pdf_logo.gif" border="0" alt="pdf">'}
if ($links =~ /.html/) {print '<img src="/images/www.gif" border="0" alt="html">'}

print "
	   $links
           <br><br>
";
if ($plinks ne '') {print "           <b>Available to purchase:</b> $plinks\n"; }
print "
         </p>
	 <p>
	 <b>Abstract:</b>
...
##    print "Author: $author_goal; Keyword: $keyword_goal; Year: $year_goal<br><br>\n";
     ...
     &get_entry;			# prime the system ...
     $pub = substr($line,1,5);
     while (&get_entry) {		# read until there ain't no more; return with $author, $title, $citation, $keyword...
       if (&match) {	        # what is it we wanted to do?
         $counter+=1;
         if ($citationt ne '') {
           &cite;    # change citationt's "<a>" to "<a href="searchpub.pl?pub=yyyya">
           print '<p align="left" class="Reference">',"\n";
           print "$citationta\n";
           chomp $links; $linkscounter+=1 if ($links ne '');
           $treesearchcounter+=1 if ($treesearch ne '');
#          print ' <img src="/images/epage_s.gif" border="0" alt="Available electronically">' if ($links ne '');
           print ' <font color="green">[Full&nbsp;text&nbsp;available]</font> ' if ($links ne '');
           if ($treesearch ne '') {
             print '
    [Available through
...
#      <img src=\"/images/epage_s.gif\" border=0>
     print "<p align=center>$counter matching $publication_s found;
      ...
      $treesearchcounter through Treesearch<br>\n";

if ($countem) {

  print "    $totaldown downloads</p>\n";
  while (($key, $value) = each(%counthash)){
     print $value.", ".$key.', "'.$titlehash{$key}.'"'."<br>\n";
  }
...
sub get_entry {
     $authors='';  $year='';     $title='';     $publisher=''; $category='';
     $abstract=''; $keyword=''; $authorblock=''; $links='';  $citationt='';
     $plinks=''; $treesearch='';
...
#   $free_text -- of title? other?
     ...
     return &null_match($author, $author_goal) && 
   ...
   <tr> 
    ...
    <td width="15%" rowspan="2" bgcolor="#FFFFFF" valign="top">
     ...
     <p>Project Leader:<br>
      ...
      <a href="https://forest.moscowfsl.wsu.edu/cgi-bin/fswepp/commform.pl?&recipient=welliot&title=Bill%20Elliot">
      <img src="/images/mail.gif" border=0 align="top" width="32" height="21" alt="email Bill"></a>
      <br>
     </p>
      <p>Contact Webmaster<br />
      <a href="https://forest.moscowfsl.wsu.edu/cgi-bin/fswepp/commform.pl?&recipient=webmaster&title=the%20Webmaster">
...
#                     "?pub=$pub" . '" target="index">';
     $citationta = substr($citationt,0,$anchor) . $substitution .
