...
#  $climate_name=$parameters{'climate_name'};
   ...
   $climate_name =~ s/^\s+//;     # http://perldoc.perl.org/perlfaq4.html#How-do-I-strip-blank-space-from-the-beginnin$
   $climate_name =~ s/\s+$//;
   $climate_name_filt=$climate_name;
   $climate_name_filt =~ s/'/`/;
   $climate_name_url = $climate_name_filt;
   $climate_name_url =~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;	# URL-encode http://support.internetconnection.net/CODE_LIBRARY/Perl_URL_Encode_and_Decode.shtml
...
##########################
   ...
   function ShowGoogleMap() {
    newin = window.open('','map',',scrollbars=yes,resizable=yes,height=1000,width=1800,status=yes')
    newin.document.open()
    ...
    newin.document.writeln('   <h3>$climate_name_filt -- Latitude $lat<sup>o</sup>, longitude $lon<sup>o</sup></h3>')    // 2009.07.22 DEH
    newin.document.writeln('   <iframe src =\"http://maps.google.com/maps?t=h&ie=UTF8&ll=$lat,$lon&q=$climate_name_url\@$lat,$lon&z=8\" width=\"90%\" height=\"90%\">')  // 2009.07.31 DEH
    newin.document.writeln('   </iframe>')
    newin.document.writeln('  </center>')
   ...
   function ShowTigerMap() {		# TIGER service diabled May 2010 per web TIGER 'more options' site
...
print '
    ...
    <form>
     <input type="submit" value="Show map" onClick="ShowTigerMap(); return false;">
     <input type="submit" value="Show map" onClick="ShowGoogleMap(); return false;">
