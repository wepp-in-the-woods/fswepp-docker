...
//    $z = array_multisort($dist, $file, $stat);
    ...
    if (array_multisort($dist, $file, $stat)) {
      ...
      if ($action == '-download') {
      echo '
    <form name="sc" ACTION="/cgi-bin/fswepp/rc/climate_ll.cli" method="post">';
      }
      else { echo '
    <form name="sc" ACTION="/cgi-bin/fswepp/rc/copypar_ll.pl" method="post">';
      }
      echo '
     ...
     <select name="station" size="'.$howmany.'">
';
      for ($j = 0; $j < $howmany; $j++) {
        list($state,$fname)=split("/",$file[$j],2);
        $STATE=strtoupper($state);
        list($CL,$type) = explode(".",$file[$j]);
        $distance=trim(sprintf("%8.2f",$dist[$j]));
        $station_t = trim($stat[$j]);
        if ($j == 0) echo '      <OPTION VALUE="'.$CL.'" selected>'.$station_t." ($distance $units2)</option>\n";
        if ($j > 0)  echo '      <OPTION VALUE="'.$CL.'">'.$station_t." ($distance $units2)</option>\n";
      }
...
';
   if ($action == '-download') {
     echo 'Number of years of climate <input type="text" name="simyears" value="30" size=4>
  <br><br>
     <input type="submit" name="submitbutton" value="DOWNLOAD CLIMATE">
     <input type="hidden" name="action" value="-download">';
   }
   else {
     echo '     <input type="submit" name="submitbutton" value="ADD TO PERSONAL CLIMATES">';
   }
   echo '
