...
function mod_tmp(obj) {
 ...
 if (isNumber(obj.value)) {
    if (obj.value < min){
         alert('Temperature must be between ' + min + ' and ' + max + unit)
         obj.value=min
    }
    if (obj.value > max) {
         alert('Temperature must be between ' + min + ' and ' + max + unit)
...
// check valid number first
 ...
 if (isNumber(obj.value)) {
    if (obj.value < min){
         alert('Precipitation must be between ' + min + ' and ' + max + unit)
         obj.value=min
    }
    if (obj.value > max) {
         alert('Precipitation must be between ' + min + ' and ' + max + unit)
...
function latlong_calc(unit) {
  ...
  var url='/swe/software/milatcon.html';
  if (unit == 'm') {url='/swe/software/kmlatcon.html'}
...
<td width=45%>
<center>
     <H2>Modify climate parameters below 
<font size=-1>[ <a href="/fswepp/docs/modpardoc.html" target="docs">Explain</a> ]
</font></H2>
...
';
print "<b>Modpar</b> version $version";
print '   (a part of <b>Rock:Clime</b>)<br>                
