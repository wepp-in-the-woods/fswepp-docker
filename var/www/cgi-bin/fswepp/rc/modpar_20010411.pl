...
#      $units		User's units scheme ('-um', 'm', '-uft' or 'ft')
#      $comefrom        WEPP:Road or Disturbed WEPP path for retreat
#      $state           
#    form:
...
#  14 December 2000 send station lat & long AND prism lat & long to form "prism"
#  02 December 1999
#  18 December 1999
#  12 April 2000
#  19 July 2000 DEH units "-um" -> "m" and "-uft" -> "ft"
#  10/31/2000 DEH Incoming PRISM longitude back to negative (W)
#                 Modify "Modify climate" to "Use these values" and add <p>
#  03/01/2001 DEH add lat long calculator a'la whitepine
#		 DEH change % column change on temperatures to degrees
#	 	 DEH change "Modify climate" button text to "Use these values"
#  03/05/2001 DEH add knowledge of cookies to determine personality
...
#   $debug = 1;
   ...
   if ($CL . $units . $comefrom . $state eq "") { # called by form (PRISM)
      &ReadParse(*parameters);
      $plat = $parameters{'platitude'};
      $plon = -1 * $parameters{'plongitude'};	# DEH 10/31/2000
...
function latlong_calc(unit) {                                                   
  ...
  var url='/fswepp/milatcon.html';                                        
  if (unit == 'm') {url='/fswepp/kmlatcon.html'}                          
...
";
   ...
   if ($debug) {print "modpar $CL $climateFile $units<p>\n";}
   print "<center>
       <table width=80%>
       <tr>
       <td width=45%>
       <CENTER>
       <H2>Climate parameters for<br>$climate_name</H2>
     ";
...
<td width=45%>
<center>
     <H2>Modify climate parameters below</H2>
     ...
     <br>(<a href="Javascript:latlong_calc(\'',$units,'\')">calculator</a>)
...
<p>
    ...
    ';		# DEH 03/05/2001
print '<input type=hidden name=latitude value=';
...
<tr><td align=right> ',$tmax[$i],'
    ...
    <td align=right><input type=text name="tx',$i+1,'" size=8 value="',$tmax[$i],'">
    <td align=right><input type=text name="tn',$i+1,'" size=8 value="',$tmin[$i],'">
    <td align=right><input type=text name="pc',$i+1,'" size=8 value="',$ppcp[$i],'" onChange="sum_pc()">
...
<td align=right>+/-<input type=text size=5 name="txd" value="0" onChange="txdeg()"><sup>o</sup>
<td align=right>+/-<input type=text size=5 name="tnd" value="0" onChange="tndeg()"><sup>o</sup>
<td align=right>+/-<input type=text size=5 name="pcp" value="0" onChange="pcpct()">%
<td align=right>+/-<input type=text size=5 name="nwp" value="0" onChange="nwpct()">%
</table>
<p>
