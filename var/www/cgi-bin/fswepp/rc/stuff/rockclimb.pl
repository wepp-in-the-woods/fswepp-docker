...
####################
    ...
    if ($arg0 . $arg1 . $arg2 . $argv3 eq "") {
      ...
      $comefrom = "";
#    if (lc($arg3) =~ /-i/) {$me = $arg3}
#    if (lc($arg2) =~ /-i/) {$me = $arg2}
#    if (lc($arg1) =~ /-i/) {$me = $arg1}
#    if (lc($arg0) =~ /-i/) {$me = $arg0}
#    $me=substr($me,2,1);
#    if ($me ne "") {
#       $me = lc(substr($me,0,1));
#       $me =~ tr/a-z/ /c;
...
print "action: '$action'<br>\n";
  ...
  if ($action ne '-download') {
    print "<h3>Personal Climates</h3><p>\n";
...
';
  ...
  print '<form name="retreat" method="post" action="',$comefrom,'">
<input type="hidden" name="units" value="',$units,'">
<input type="hidden" name="me" value="',$me,'">
...
<table width=100% border=0>
  <tr><td>Rock:Clime version ', $version,'<br>
