...
# 10 April 2000      	DEH -- Veg check biomass in t/ac
# 06 January 2000
# 04 November 1999
...
<h2>User inputs</h2>
<table border=1>
  <tr><th bgcolor=85d2d2>Location <td colspan=5><b>$climate_name
  <tr><th bgcolor=85d2d2>Soil texture <td colspan=5><b>$soil_type{$soil}
  <tr><th bgcolor=85d2d2>Element<th bgcolor=85d2d2>Treatment
      <th bgcolor=85d2d2>Gradient<BR> (%)
      <th bgcolor=85d2d2>Length<BR> ($units)
      <th bgcolor=85d2d2>Area<BR> ($areaunits)
      <th bgcolor=85d2d2>Cover<BR> (%)
  <tr>      <th align=left rowspan=2 bgcolor=85d2d2>Upper
<td rowspan=2>$treatments{$treat1}
      <td align=right>$ofe1_top_slope
      <td align=right rowspan=2>$user_ofe1_length
      <td align=right rowspan=4>$user_ofe_area
      <td align=right rowspan=2>$ofe1_pcover
  <tr><td align=right>$ofe1_mid_slope
  <tr>      <th align=left rowspan=2 bgcolor=85d2d2>Lower
<td rowspan=2>$treatments{$treat2}
      <td align=right>$ofe2_mid_slope
      <td align=right rowspan=2>$user_ofe2_length
      <td align=right rowspan=2>$ofe2_pcover
  <tr><td align=right>$ofe2_bot_slope
  <tr><td>
      <th bgcolor=85d2d2>Width ($units)
      <td><br>
      <td colspan=2>$rofe_width
</table>
<p>";
...
print "<p><hr><p>
<h2>Mean annual averages for $simyears years</h2>
<table border=0 cellpadding=4>
  <tr><td>$user_precip
      <td>$pcp_unit
      <td>precipitation from 
      <td align=right> $storms
      <td><a onMouseOver=\"window.status='There were $storms storms in $simyears year(s)';return true;\"
             onMouseOut=\"window.status='';return true;\">
      storms</a>
  <tr><td>$user_rro
      <td> $pcp_unit
      <td> runoff from rainfall from
      <td align=right> $rainevents
      <td> events
  <tr><td>$user_sro
      <td> $pcp_unit
      <td> runoff from snowmelt or winter rainstorm from
      <td align=right>$snowevents
      <td> events
  <tr><td valign=bottom>$user_asyra
      <td>$rate
      <td valign=bottom>upland erosion rate ($syra kg/m<sup>2</sup>)
  <tr><td valign=bottom>$user_asypa
      <td>$rate
      <td valign=bottom>sediment leaving profile ($sypa kg/m width)
</table>
</center>
";
...
#####
       if (lc($action) =~ /vegetation/) {
         ...
         &parsead;
         print "<center>\n";
         print "<p><hr><p><h2>Return period analysis based on $simyears years</h2>\n";
         print "<p>";

         print "
<table border=1 cellpadding=3>
  <tr><th bgcolor=85d2d2>Return<br>Period
      <th bgcolor=85d2d2>Precipitation<br>($pcp_unit)
      <th bgcolor=85d2d2>Runoff<br>($pcp_unit)
      <th bgcolor=85d2d2>Erosion<br>($rate)
      <th bgcolor=85d2d2>Sediment<br>($rate)
";
...
#     $asyp = sprintf "%.2f", $sed_del[$rp_year-1] * $ofe_width / (100000 * $ofe_area) * $rcf;	# kg/m width * m width * (1 t / 1000 kg) / area-in-ha
      ...
      print "  <tr><th bgcolor=85d2d2>
        <a onMouseOver=\"window.status='For year with $rp_year_text[$ii] largest values';return true;\"
           onMouseOut=\"window.status='';return true;\">$rp year</a>
      <td align=right>$user_pcpa
      <td align=right>$user_ra
      <td align=right>$asyr
      <td align=right>$asyp
      ";
      $ii += 1;
...
print "  <tr><th bgcolor=yellow>Average
      <td align=right>$user_avg_pcp
      <td align=right>$user_avg_ro
      <td align=right>$user_asyra
      <td align=right>$user_asypa
";
         $base_size=100;
...
<tr><th align=right>Probability there is runoff
    <td>"; printf "%.0f",(1-$prob_no_runoff)*100; print " %
    <td>
        ...
        <a onMouseOver=\"window.status='$nzra year(s) in $simyears had no runoff';return true;\"
           onMouseOut=\"window.status='';return true;\">
        <img src=\"/fswepp/images/green.gif\" height=15 width=",($prob_no_runoff)*$base_size,"></a>

<tr><th align=right>Probability there is erosion
    <td>"; printf "%.0f",(1-$prob_no_erosion)*100; print " %
    <td><a onMouseOver=\"window.status='$nnzdetach year(s) in $simyears had erosion';return true;\"
           onMouseOut=\"window.status='';return true;\">
        ...
        <a onMouseOver=\"window.status='$nzdetach year(s) in $simyears had no erosion';return true;\"
           onMouseOut=\"window.status='';return true;\">
        <img src=\"/fswepp/images/green.gif\" height=15 width=",($prob_no_erosion)*$base_size,"></a>

<tr><th align=right>Probability of sediment delivery
    <td>"; printf "%.0f", (1-$prob_no_sediment)*100; print " %
    <td><a onMouseOver=\"window.status='$nnzsed_del year(s) in $simyears had sediment delivery';return true;\"
           onMouseOut=\"window.status='';return true;\">
        ...
        <a onMouseOver=\"window.status='$nzsed_del year(s) in $simyears had no sediment delivery';return true;\"
           onMouseOut=\"window.status='';return true;\">
        <img src=\"/fswepp/images/green.gif\" height=15 width=",($prob_no_sediment)*$base_size,"></a>
</table>
</center>
";
...
# print "<hr width=50%> \n";
       ...
       if ($outputi==1){
         print '<HR>
<P>
<CENTER><H3>Generated slope file</H3></CENTER>
<PRE>
';
...
#-----------------------------------------
     ...
     if ($outputf==1) {
       ...
       while (<weppout>){    # skip 'til "ANNUAL AVERAGE SUMMARIES"
         if (/ANNUAL AVERAGE SUMMARIES/) {last}
       }
       print;
       while (<weppout>){print}
       close weppout;
...
print ' <a href="https://',$wepphost,'/fswepp/history/wdrver.html">';
print "$version </a>
based on WEPP $weppver
<br><tt>https://$wepphost/fswepp</tt><br>";
&printdate;
print "</blockquote>\n</BODY>\n</HTML>\n";
...
#      $treat1 eq "shrub") {
#    @values = split "\t", $in;
#    $ke = ($values[8] * $fcover1) + (0.2 * (1-$fcover1));
#    $values[8] = $ke;
#    for $j (0..7) {
#      print SOLFILE $values[$j],"  ";
#    }
#    print SOLFILE $values[8],"\n";
#} 
#  else {
#    $in =~ tr/\t/  /;     # translate tabs to spaces
    print SOLFILE $in,"\n";
#  }
   $in = <SOILDB>;
#  $in =~ tr/\t/  /;    # tabs to spaces
   print SOLFILE $in;
   close SOILDB;
...
#      $treat eq "shrub") {
#    @values = split "\t", $in;
#    $ke = ($values[8] * $fcover2) + (0.2 * (1-$fcover2));
#    $values[8] = $ke;
#    for $j (0..7) {
#      print SOLFILE $values[$j],"  ";
#    }
