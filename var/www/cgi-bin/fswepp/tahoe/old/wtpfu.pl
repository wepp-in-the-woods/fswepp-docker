...
#
# Tahoe Basin Sediment Model workhorse * with phosphate *
# Reads user input from tahoe.pl, runs WEPP, parses output files

# Needed update: automatic history popup

# 2011.11.27 DEH -- Add phosphate analysis, tidy up code and displays
...
#  $version = '2012.04.09';	# move phosphate inputs to input table; interaction not needed
#  $version = '2011.12.09';     # combine phosphate table with precipitation/erosion table
#  $version = '2011.11.30';	# read WEPP water file to determine lateral flow
#  $version = '2011.11.27';	# Continue putting phosphate table in
#  $version = '2011.11.18';	# Start putting phosphate table in
...
# fines
   $fines_upper=$parameters{'fines_upper'};	# upper limit for fines analysis (microns) [4..62.5]

# phosphate concentration parameter input values

   $sr_conc = $parameters{'sr_conc'};			# surface runoff phosphate concentration
   $lf_conc = $parameters{'lf_conc'};			# lateral flow phosphate concentration
   $sed_conc= $parameters{'sed_conc'};			# sediment phosphate concentration

   $sr_conc_in = $sr_conc;    $lf_conc_in = $lf_conc;    $sed_conc_in = $sed_conc;
   $sr_conc_color[0] = 'red'; $sr_conc_color[1] = 'black';
   $lf_conc_color[0] = 'red'; $lf_conc_color[1] = 'black';
   $sed_conc_color[0]= 'red'; $sed_conc_color[1]= 'black';

   $sr_conc_min = 0;  $sr_conc_max = 4;    $sr_conc_def = 1;	# mg/l
   $lf_conc_min = 0;  $lf_conc_max = 4;    $lf_conc_def = 1.5;	# mg/l
...
#  if ($sr_conc !~ /^-?(?:\d+(?:\.\d*)?&\.\d+)$/) {$sr_conc = $sr_conc_def; $sr_conc_ok=0};	# match decimal number http://stackoverflow.com/questions/3803850/how-do-i-decide-if-a-variable-is-numeric-in-perl
   ...
   if (looks_like_number $sed_conc) {
     if ($sed_conc <= $sed_conc_min) {$sed_conc = $sed_conc_min; $sed_conc_ok=0}
     if ($sed_conc >= $sed_conc_max) {$sed_conc = $sed_conc_max; $sed_conc_ok=0}
   } else {
...
<!--             phosphate table calculations start         -->
...
<!--             phosphate table calculations end         -->
...
#      }
   ...
   if ($simyears == '1') {$yearyears='year'} else {$yearyears = 'years'}
           $_ = <weppout>; # 
...
#    $user_ofe1_length
     ...
     ($sumP,$sumRM,$sumQ,$sumEp,$sumEs,$sumDp,$sumLatqcc,$sumTotal,$firstTotalSoilWater,$lastTotalSoilWater,$OFEsoff) = &WaterBalanceSum($WatBalFile);
...
# Average lateral flow from toe
     $lateral_flow = ($sumLatqcc * $user_ofe2_length / $user_total_length) / $years2sim;	# mm
  ...
  <center>
   ...
   <table border=1><tr><td>
    <table border=0 cellpadding=5 bgcolor='#faf8cc'>
     <tr><th colspan=6 bgcolor='white'>User inputs</th></tr>
     <tr>
      <td colspan=5><font face='Arial, Geneva, Helvetica' size=-1>$description</font></td>
      <td id='years'><font face='Arial, Geneva, Helvetica'>$simyears $yearyears</font></td>
     </tr>
     <tr>
...
";
     ...
     <tr>
      <th bgcolor='#85d2d2'><font face='Arial, Geneva, Helvetica'>Element</font></th>
     ...
     <tr>
      <th align=right rowspan=2 bgcolor='#85d2d2'><font face='Arial, Geneva, Helvetica'>Upper</font></th>
      <td rowspan=2 bgcolor='#FAF8CC'><font face='Arial, Geneva, Helvetica'>$treatments{$treat1}</font></td>
      <td align=right bgcolor='#FAF8CC'><font face='Arial, Geneva, Helvetica'>$ofe1_top_slope</font></td>
      <td align=right rowspan=2 bgcolor='#FAF8CC' id='len_top'><font face='Arial, Geneva, Helvetica'>$user_ofe1_length</font></td>
      <td align=right rowspan=2 bgcolor='#FAF8CC'><font face='Arial, Geneva, Helvetica'>$ofe1_pcover</font></td>
      <td align=right rowspan=2 bgcolor='#FAF8CC'><font face='Arial, Geneva, Helvetica'>$ofe1_rock</font></td>
     </tr>
     <tr>
      <td align=right bgcolor='#FAF8CC'><font face='Arial, Geneva, Helvetica'>$ofe1_mid_slope</font></td>
     </tr>
     <tr>
      ...
      <td align=right rowspan=2 bgcolor='#FAF8CC' id='len_bot'><font face='Arial, Geneva, Helvetica'>$user_ofe2_length</td>
     ...
     <tr>
      ...
      <td colspan=5 bgcolor='#FAF8CC'><font face='Arial, Geneva, Helvetica'>$soil_type{$soil}</font></td>
     </tr>
     <tr>
      <th bgcolor='#85d2d2'><font face='Arial, Geneva, Helvetica'>Phosphate concentration</font></th>
      <td colspan=5 bgcolor='#FAF8CC'>
       <font face='Arial, Geneva, Helvetica'>
        <font color='",$sr_conc_color[$sr_conc_ok],"'>$sr_conc_in</font> mg/l surface;
        <font color='",$sr_conc_color[$lf_conc_ok],"'>$lf_conc_in</font> mg/l lateral flow;
        <font color='",$sr_conc_color[$sed_conc_ok],"'>$sed_conc_in</font> mg/kg sediment</td> 
       </font
      </td>
     </tr>
    </table>
...
if ($units eq 'm') {
   ...
   $rate = 't ha<sup>-1</sup>';
   $pcp_unit = 'mm';
   $phosphate_delivery_units = 'kg/ha'; $p_conv = 1
} else {	# $units eq 'ft'
   ...
   $rate = 't ac<sup>-1</sup>';
   $pcp_unit = 'in.';
   $phosphate_delivery_units = 'lb/ac'; $p_conv = 0.8922;	# unit conversion: kg/ha * 0.8922 = lb/ac [wolfram]
}
...
# $lf_conc = 1.5;
# $sed_conc = 100;
$sr_delivery=($user_rro+$user_sro) * $sr_conc / 100;		# user units -- kg/ha (mm * mg/l) or lb/ac
$lf_delivery=$lateral_flow_u * $lf_conc / 100;			# user units -- kg/ha (mm * mg/l) or lb/ac
$sed_delivery=$user_asypa * $sed_conc * $SSA_ratio / 1000;	# user units -- kg/ha or lb/ac
$total_phosphate=$sr_delivery+$lf_delivery+$sed_delivery;	# user units -- kg/ha (t/ha * mg/kg) or lb/ac
...
##
    ...
    ($class[1],$diam,$spgr,$sand_pct,$silt_pct,$clay_pct[1],$om_pct,$sedfrac,$frac_exiting[1],$rest) = split ' ',$class1,9;
    ($class[2],$diam,$spgr,$sand_pct,$silt_pct,$clay_pct[1],$om_pct,$sedfrac,$frac_exiting[2],$rest) = split ' ',$class2,9;
    ($class[3],$diam,$spgr,$sand_pct,$silt_pct,$clay_pct[1],$om_pct,$sedfrac,$frac_exiting[3],$rest) = split ' ',$class3,9;
    ($class[4],$diam,$spgr,$sand_pct,$silt_pct,$clay_pct[1],$om_pct,$sedfrac,$frac_exiting[4],$rest) = split ' ',$class4,9;
    ($class[5],$diam,$spgr,$sand_pct,$silt_pct,$clay_pct[1],$om_pct,$sedfrac,$frac_exiting[5],$rest) = split ' ',$class5,9;

    $frac_exiting[1]+=0; $frac_exiting[2]+=0; $frac_exiting[3]+=0; $frac_exiting[4]+=0; $frac_exiting[5]+=0;

    $pct_clay[1] = 100   * $frac_exiting[1];	# 100   * 0.006
    $pct_clay[2] =   0;				#   0   * 0.050
    $pct_clay[3] =  20   * $frac_exiting[3];	#  20   * 0.041
    $pct_clay[4] =   7.5 * $frac_exiting[4];	#   7.5 * 0.115
    $pct_clay[5] =   0; 			#   0   * 0.788

    $pct_silt[1] =   0;				#   0   * 0.006
    $pct_silt[2] = 100   * $frac_exiting[2];	# 100   * 0.050
    $pct_silt[3] =  80   * $frac_exiting[3];	#  80   * 0.041
    $pct_silt[4] =   7.1 * $frac_exiting[4];	#   7.1 * 0.115
    $pct_silt[5] =   0; 			#   0   * 0.788

    $sed_del = $user_asypa;	# average annual sediment delivery in user units (t/ac or t/ha)

    $total_clay_in_del_sed = $pct_clay[1]+$pct_clay[3]+$pct_clay[4];	# % pct
    $total_clay_in_del_sed_ratio = $total_clay_in_del_sed/100;
    $total_clay_in_del_sed_ratio_f = sprintf "%0.2f",$total_clay_in_del_sed_ratio;
    $total_silt_in_del_sed = $pct_silt[2]+$pct_silt[3]+$pct_silt[4];	# % pct
    $total_silt_in_del_sed_ratio = $total_silt_in_del_sed/100;
    $total_silt_in_del_sed_ratio_f = sprintf "%0.2f",$total_silt_in_del_sed_ratio;

    $total_clay_del = $total_clay_in_del_sed_ratio * $sed_del;	# Total clay delivered: $total_clay_del t/ac or t/ha
    $total_silt_del = $total_silt_in_del_sed_ratio * $sed_del;	# Total silt delivered: $total_silt_del t/ac or t/ha
    $total_clay_del_f = sprintf "%0.2f",$total_clay_del;
    $total_silt_del_f = sprintf "%0.2f",$total_silt_del;
...
#  USDA silt size definition is 4.0 to 62.5 microns (= user input limits)
   ...
   $fines_upper = $fines_upper_limit if ($fines_upper > $fines_upper_limit);
   $fines_upper = $fines_lower_limit if ($fines_upper < $fines_lower_limit);
#  $upper = 10;			# ******* user input 
   $numer = $fines_upper - $fines_lower_limit;
   $silt_ratio = $numer / $denom;

   $silt_frac_less = $silt_ratio * $total_silt_del;	# Silt fraction less than $fines_upper microns, t/ac or t/ha

if ($units eq 'm') {
   $fines_units='kg/ha';
   $silt_frac_less *= 1000;				# Silt fraction less than $fines_upper microns, kg/ha
} else {
#  $fines_units='t/ac';
#  $fines_units_alt='lb/ac';
   $fines_units='lb/ac';
   $silt_frac_less *= 2000;				# Silt fraction less than $fines_upper microns, lb/ac
}
   $silt_frac_less_del = $silt_frac_less*$sed_del;
   $silt_frac_less_del_f = sprintf "%0.2f",$silt_frac_less_del;
...
#   <p><hr><p>
   ...
   <h3>Precipitation, runoff, erosion, phosphate for $simyears $yearyears</h3>
   <table border=1>
   <tr><th>Precipitation, runoff, erosion, phosphate, fines analyses</th></tr>
   <tr><td>
    <table border=0 cellpadding=4 bgcolor='#e0ffff'>
     <tr>
      ...
      <th colspan=4 bgcolor='#85d2d2' style='border-left:solid 4px purple'>Phosphate Analysis</th>
     </tr>
     <tr>
      <th colspan=2 bgcolor='#85d2d2' style='border-left:solid 4px purple'><font size=-1>Concentration</font></th>
      <th colspan=2 bgcolor='#85d2d2'><font size=-1>Delivery</font></th>
     </tr>
     <tr>
      ...
      <th colspan=2 style='border-left:solid 4px purple'><font face='Arial, Geneva, Helvetica'></font></th>
      <th colspan=2><font face='Arial, Geneva, Helvetica'></font></th>
     </tr>
     <tr>
      <th align=right bgcolor='#85d2d2'><font face='Arial, Geneva, Helvetica' size=-1>Runoff from rainfall</font></th>
      <td align=right><font face='Arial, Geneva, Helvetica'>$rainevents</font></td>
      <td><font face='Arial, Geneva, Helvetica'>events</font></td>
      <td align=right id='avg_rro'><font face='Arial, Geneva, Helvetica'>$user_rro</font></td>   <!-- rro  -->
      <td><font face='Arial, Geneva, Helvetica'>$pcp_unit</font></td>
      <td rowspan=2 bgcolor=faf8cc align=right id='srConc' style='border-left:solid 4px purple'><font face='Arial, Geneva, Helvetica'>$sr_conc</font></td>
      <td rowspan=2><font face='Arial, Geneva, Helvetica'>mg/l</font></td>
      <td rowspan=2 align=right id='srDel' onmouseover='explain(1);markSrCalc(1)' onmouseout='explain(0);markSrCalc(0)'>
       <div title='",$sr_delivery*$p_conv," = ($user_rro + $user_sro) * $sr_conc / 100'>
        <font face='Arial, Geneva, Helvetica'><span id='sr_delivery'>",sprintf("%.3f",$sr_delivery*$p_conv),"</span></font>
       </div>
      </td>
      <td rowspan=2><font face='Arial, Geneva, Helvetica'>",$phosphate_delivery_units,"</font></td>
     </tr>
     <tr>
      ...
      <td><font face='Arial, Geneva, Helvetica'>events</font></td>
      <td align=right id='avg_sro'><font face='Arial, Geneva, Helvetica'>$user_sro</font></td>   <!-- sro  -->
      <td><font face='Arial, Geneva, Helvetica'>$pcp_unit</font></td>
     </tr>
     <tr>
      <th align=right valign=bottom bgcolor='#85d2d2'><font face='Arial, Geneva, Helvetica' size=-1>Lateral flow</font></th>
      <td align=right id='sumLat'><font face='Arial, Geneva, Helvetica'>",sprintf("%.2f",$sumLatqcc*$lf_conv),"</font></td>
      <td><font face='Arial, Geneva, Helvetica'>$pcp_unit</font></td>
      <td align=right id='lfAmt' onmouseover='explain(5);markLatCalc(1)' onmouseout='explain(-1);markLatCalc(0)'>
       <font face='Arial, Geneva, Helvetica'>",sprintf('%.2f',$lateral_flow_u),"</font>
      </td>
      <td><font face='Arial, Geneva, Helvetica'>$pcp_unit</font></td>
      <td align=right bgcolor='#faf8cc' id='lfConc' style='border-left:solid 4px purple'><font face='Arial, Geneva, Helvetica'>$lf_conc</font></td>
      <td><font face='Arial, Geneva, Helvetica'>mg/l</font></td>
      <td align=right id='lfDel' onmouseover='explain(2);markLfCalc(1)' onmouseout='explain(-1);markLfCalc(0)'>
       <div title='",$lf_delivery*$p_conv," = $lateral_flow_u * $lf_conc / 100'>
        <font face='Arial, Geneva, Helvetica'>
         <span id='lf_delivery'>",sprintf("%.3f",$lf_delivery*$p_conv),"</span>
        </font>
       </div>
      </td>
      <td><font face='Arial, Geneva, Helvetica'>",$phosphate_delivery_units,"</font></td>
     </tr>
     <tr>
      ...
      <td style='border-left:solid 4px purple'>&nbsp; </td>
     </tr>
     <tr>
      ...
      <td></td><td></td>
      <td align=right id='sedAmt' valign=bottom id='sed'><font face='Arial, Geneva, Helvetica'>$user_asypa</font></td>   <!-- sed  -->
      <td><font face='Arial, Geneva, Helvetica'>$rate</font></td>
      <td align=right bgcolor='#faf8cc' id='sedConc' style='border-left:solid 4px purple'><font face='Arial, Geneva, Helvetica'>$sed_conc</font></td>
      <td><font face='Arial, Geneva, Helvetica'>mg/kg</font></td>
      <td align=right id='sedDel' onmouseover='explain(3);markSedCalc(1)' onmouseout='explain(-1);markSedCalc(0)'>
       <div title='",$sed_delivery * $p_conv," = $sed_del * $sed_conc * $ssa_ratio'>
        <font face='Arial, Geneva, Helvetica'>
         <u>
          <span id='sed_delivery'>",sprintf("%.3f",$sed_delivery*$p_conv),"</span>
         </u>
        </font>
       </div>
      </td>
     ...
     <tr>
      <th align=right valign=bottom><font face='Arial, Geneva, Helvetica' size=-1></font></th>
      <td></td><td></td>
      <td align=right valign=bottom><font face='Arial, Geneva, Helvetica'></font></td>
      <td><font face='Arial, Geneva, Helvetica'></font></td>
      <th valign=bottom colspan=2 style='border-left:solid 4px purple'><font face='Arial, Geneva, Helvetica'>Total</font></th>
      <td align=right onmouseover='explain(4);markDeliveries(1)' onmouseout='explain(-1);markDeliveries(0)'>
       <div title='",$total_phosphate*$p_conv," = ",$sr_delivery*$p_conv," + ",$lf_delivery*$p_conv," + ",$sed_delivery*$p_conv,"'>
        <font face='Arial, Geneva, Helvetica'>
         <span id='total'>",sprintf("%.3f",$total_phosphate*$p_conv),"</span>
        </font>
       </div>
      </td>
      <td valign=bottom><font face='Arial, Geneva, Helvetica'>",$phosphate_delivery_units,"</font></td>
     </tr>
<!--   FINES START  -->
     <tr>
      <th align=right valign=bottom><font face='Arial, Geneva, Helvetica' size=-1></font></th>
      <th colspan=4 bgcolor='#85d2d2'><font face='Arial, Geneva, Helvetica'><b>Fines analysis</b></font></th>
     ...
     <tr>
      <th align=right valign=bottom><font face='Arial, Geneva, Helvetica' size=-1></font></th>
      <th colspan=4 align=right bgcolor='#85d2d2'>Clay</th>
      <td align=right valign=bottom id='ClayRatio'>
       <DIV TITLE='$total_clay_in_del_sed_ratio = $frac_exiting[1] + 20 * $frac_exiting[3] + 7.5 * $frac_exiting[4]'>
        <font face='Arial, Geneva, Helvetica'>
      ...
      <td><font face='Arial, Geneva, Helvetica'></font></td>
      <td onmouseover='markAvgSed(1);markClayRatio(1)' onmouseout='markAvgSed(0);markClayRatio(0)'>
       <DIV TITLE='$total_clay_del = $total_clay_in_del_sed_ratio * $sed_del'>
        <font face='Arial, Geneva, Helvetica' size=-1>
         $total_clay_del_f
        </font>
       </div>
      </td>
      <td align=right><font face='Arial, Geneva, Helvetica'><span id='clay'>$fines_units</span></font></td>
     </tr>
     <tr>
      <th align=right valign=bottom><font face='Arial, Geneva, Helvetica' size=-1></font></th>
      <th colspan=4 align=right bgcolor='#85d2d2'>Silt</th>
      <td align=right valign=bottom id='SiltRatio'>
       <DIV TITLE='$total_silt_in_del_sed_ratio = [$frac_exiting[2] + 80 * $frac_exiting[3] + 7.1 * $frac_exiting[4]]'>
        <font face='Arial, Geneva, Helvetica'>",$total_silt_in_del_sed_ratio_f,"
        </font>
       </div>
      </td>
      <td><font face='Arial, Geneva, Helvetica'></font></td>
      <td onmouseover='markAvgSed(1);markSiltRatio(1)' onmouseout='markAvgSed(0);markSiltRatio(0)'>
       <DIV TITLE='$total_silt_del = $total_silt_in_del_sed_ratio * $sed_del'>
        <font face='Arial, Geneva, Helvetica' size=-1>$total_silt_del_f
        </font>
       </div>
      </td>
      <td align=right>$fines_units</font></td>
     </tr>
     <tr>
      <th align=right valign=bottom><font face='Arial, Geneva, Helvetica' size=-1></font></th>
      <td></td>
      <td></td>
      <th colspan=2 bgcolor='#85d2d2'>Silt &lt; $fines_upper microns</th>
      <td align=right valign=bottom id='SiltLessRatio'>
       <DIV TITLE='$silt_frac_less = [($fines_upper - $fines_lower_limit)/($fines_upper_limit - $fines_lower_limit) * $total_silt_del]'>
        <font face='Arial, Geneva, Helvetica'>
          $silt_frac_less_f
        </font>
       </div>
      ...
      <td><font face='Arial, Geneva, Helvetica'></font></td>
      <td onmouseover='markAvgSed(1);markSiltLessRatio(1)' onmouseout='markAvgSed(0);markSiltLessRatio(0)'>
       <DIV TITLE='$silt_frac_less_del = $silt_frac_less * $sed_del'>
        <font face='Arial, Geneva, Helvetica' size=-1>$silt_frac_less_del_f
        </font>
       </div>
      </td>
      <td align=right><font face='Arial, Geneva, Helvetica'>$fines_units</font></td>
     </tr>
     <tr>
      <th align=right valign=bottom><font face='Arial, Geneva, Helvetica' size=-1></font></th>
      <th colspan=4 align=right bgcolor='#85d2d2'>Total &lt; $fines_upper micron</th>
...
<!--  FINES END -->
     <tr>
      ...
      <td align=right onmouseover='explain(4);markDeliveries(1)' onmouseout='explain(-1);markDeliveries(0)'><font face='Arial, Geneva, Helvetica'><span id='total'></span></font></td>
...
#   print $class[1],' -- ',$frac_exiting[1];	# 1 -- 0.006
#   print $class[2],' -- ',$frac_exiting[2];	# 2 -- 0.050
#   print $class[3],' -- ',$frac_exiting[3];	# 3 -- 0.041
#   print $class[4],' -- ',$frac_exiting[4];	# 4 -- 0.115
#   print $class[5],' -- ',$frac_exiting[5];	# 5 -- 0.788
...
#   $sed_del = $avg_sed;		# t/ha ************* CHECK; convert to t/ac if 'ft'
    $sed_del = $user_asypa;

    $total_clay_in_del_sed = $pct_clay[1]+$pct_clay[3]+$pct_clay[4];	# % pct
    $total_silt_in_del_sed = $pct_silt[2]+$pct_silt[3]+$pct_silt[4];	# % pct

    $total_clay_del = $total_clay_in_del_sed / 100 * $sed_del;
    $total_silt_del = $total_silt_in_del_sed / 100 * $sed_del;

if ($units eq 'ft') {

print "Sediment delivered: $sed_del t/ac\n";

print "Clay fraction in delivered sediment: ", $total_clay_in_del_sed/100,"\n";
print "Total clay delivered: $total_clay_del t/ac -- ",2000*$total_clay_del," lb/ac\n"; 

print "Total silt fraction in delivered sediment: ", $total_silt_in_del_sed/100,"\n";
print "Total silt delivered: $total_silt_del t/ac -- ",2000*$total_silt_del," lb/ac\n";
...
if ($units eq 'ft') {
   print "Silt fraction less than $fines_upper microns: ", $ratio * $total_silt_del,"\n";
} else {
   print "Silt fraction less than $fines_upper microns: ", $ratio * $total_silt_del," [($fines_upper - $fines_lower_limit)/($fines_upper_limit - $fines_lower_limit) * $total_silt_del]\n";
}
...
#         &parsead;		# comment out (we did so back on line 905; don't do it again!   2012.02.21
#    <p><hr><p>
         print "
   <center>
    <h3>Return period analysis<br>
        based on $simyears&nbsp;$yearyears of climate</h3>
    <p>
...
print "
   ...
   <h3>Probabilities of occurrence<br>first year following disturbance<br>
       based on $simyears&nbsp;$yearyears of climate</h3>
    <table border=0 cellpadding=4 bgcolor='#e0ffff'>
     <tr>
     ...
     print " %</font></td>
      <td><font face='Arial, Geneva, Helvetica'>
        ...
        <a onMouseOver=\"window.status='$nzra year(s) in $simyears had no runoff';return true;\"
           onMouseOut=\"window.status='';return true;\">
        <img src=\"/fswepp/images/green.gif\" height=15 width=",($prob_no_runoff)*$base_size,"></a></font></td>
     </tr>
     <tr>
      ...
      <td><font face='Arial, Geneva, Helvetica'>
       <a onMouseOver=\"window.status='$nnzdetach year(s) in $simyears had erosion';return true;\"
        ...
        <a onMouseOver=\"window.status='$nzdetach year(s) in $simyears had no erosion';return true;\"
           onMouseOut=\"window.status='';return true;\">
        <img src=\"/fswepp/images/green.gif\" height=15 width=",($prob_no_erosion)*$base_size,"></a></font></td>
     </tr>
     <tr>
     ...
     print " %</font></td>
      <td><font face='Arial, Geneva, Helvetica'>
          ...
          <a onMouseOver=\"window.status='$nzsed_del year(s) in $simyears had no sediment delivery';return true;\"
             onMouseOut=\"window.status='';return true;\">
          <img src=\"/fswepp/images/green.gif\" height=15 width=",($prob_no_sediment)*$base_size,"></a></font></td>
     </tr>
     <tr>
      <td><font size=-2>
      ";
...
";
   ...
   open SOILDB, "<data/soilbase.tahoe";
...
# --- SSA enrichment ratio ---
       if (/Average annual SSA enrichment ratio leaving profile = *([0-9.]+)/) {
         $SSA_ratio = $1;
       }
#  -- for FINES --
       if (/SEDIMENT CHARACTERISTICS/) {
         $dummy = <AD>; #
         $dummy = <AD>; #     Sediment particle information leaving profile
         $dummy = <AD>; #-------------------------------------------------------------------------------
         $dummy = <AD>; #                                 Particle Composition         Detached Fraction
         $dummy = <AD>; #Class  Diameter  Specific  ---------------------------------  Sediment  In Flow
         $dummy = <AD>; #         (mm)    Gravity   % Sand   % Silt   % Clay   % O.M.  Fraction  Exiting
         $dummy = <AD>; #-------------------------------------------------------------------------------
         $class1 = <AD>; #  1     0.002      2.60      0.0      0.0    100.0    100.0     0.005    0.006
         $class2 = <AD>; #  2     0.010      2.65      0.0    100.0      0.0      0.0     0.044    0.050
         $class3 = <AD>; #  3     0.030      1.80      0.0     80.0     20.0     20.0     0.036    0.041
         $class4 = <AD>; #  4     0.300      1.60     85.4      7.1      7.5      7.5     0.101    0.115
         $class5 = <AD>; #  5     0.200      2.65    100.0      0.0      0.0      0.0     0.814    0.788
       }
     }
     close AD;

     chomp $class5; chomp $class4; chomp $class3; chomp $class2; chomp $class1;
...
# from ww.pl 156990 2011-03-11 16:13 ww.pl  forest:/srv/www/cgi-bin/fswepp/wasp>
...
# ($sumP,$sumRM,$sumQ,$sumEp,$sumEs,$sumDp,$sumLatqcc,$sumTotal,$firstTotalSoilWater,$lastTotalSoilWater,$OFEsoff)
 ...
 my $OFE, $JulDay, $SimYr, $P, $RM, $Q, $Ep, $Es, $Dp, $UpStrmQ, $latqcc, $TotSoilWater, $frozwt;	# read from file
