...
#   $lines = 1 if ($lines < 1);
   ...
   $lines=150000;			# DEH 2021

   @results=`tail -$lines working/_$year/we.log`;
   @results=reverse(@results);
   $wc  = `wc working/_$year/we.log`;
   @words = split " ", $wc;
   $runs = @words[0];
...
print "<HTML>
 <HEAD>
  <TITLE>ERMiT run log</TITLE>
 </head>
 <body onLoad='self.focus()'>
  <font face='tahoma, arial, sans serif'>
   <h3>$lines most recent of $runs ERMiT runs in $year</h3>
   <pre>
 @results
   </pre>
  </font>
