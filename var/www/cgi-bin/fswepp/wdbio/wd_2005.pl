...
# 2005.09.12 DEH -- Expand input parse field for 'Off Site Effects'
# 2004.07.01 DEH No calibration (no changes)
  ...
  <script>
    function showslopefile() {
     ...
     if (filewindow && filewindow.open && !filewindow.closed) {
      ...
      filewindow.document.writeln("  <title>WEPP slope file<\/title>")
      filewindow.document.writeln(" <\/head>")
...
';
    ...
    function showsoilfile() {
     ...
     if (filewindow && filewindow.open && !filewindow.closed) {
      filewindow.focus
      filewindow.document.writeln("<head><title>WEPP soil file<\/title><\/head>")
...
';
    ...
    function showresponsefile() {
     ...
     if (filewindow && filewindow.open && !filewindow.closed) {
      filewindow.focus
      filewindow.document.writeln("<head><title>WEPP response file<\/title><\/head>")
...
';
    ...
    function showvegfile() {
     ...
     if (filewindow && filewindow.open && !filewindow.closed) {
      filewindow.focus
      filewindow.document.writeln("<head><title>WEPP vegetation file<\/title><\/head>")
      filewindow.document.writeln("<body><pre>")
';
      $line = 0;
...
#       last if ($line > 100);
    ...
    function showextendedoutput() {
     ...
     filewindow.focus
     filewindow.document.writeln("<head><title>WEPP output file<\/title><\/head>")
     filewindow.document.writeln("<body><font face=\'courier\'><pre>")
';
    ...
    function showcligenparfile() {
     ...
     if (filewindow && filewindow.open && !filewindow.closed) {
      filewindow.focus
      filewindow.document.writeln("<head><title>WEPP weather file<\/title><\/head>")
      filewindow.document.writeln("<body><pre>")
';
...
popupwindow.document.writeln('     </tr>')
popupwindow.document.writeln('     <tr>')
popupwindow.document.writeln('      <th bgcolor="85d2d2"><font face="arial, helvetica, sans serif">2004.07.01</font></th>')
popupwindow.document.writeln('      <td></td>')
popupwindow.document.writeln('      <td><font face="arial, helvetica, sans serif">Follow "No Decomposition Method to Ensure Cover is Correct" (W.J. Elliot, January 2004).')
popupwindow.document.writeln('      This means no user vegetation calibration is required, but different assumptions are made, so results will vary from earlier versions.</font></td>')
popupwindow.document.writeln('     </tr>')
popupwindow.document.writeln('     <tr>')
...
#          $_ = <weppout>; if ($syp eq "") {$syp = substr $_,10,9}
           $_ = <weppout>; if ($syp eq "") {$syp = substr $_,1,18}	# 2005.09.12 DEH
           $_ = <weppout>;
           $_ = <weppout>;
           last;
         }
   ...
   <h3>User inputs</h3>
    <table border=1>
     <tr>
      ...
      <td colspan=5><font face='Arial, Geneva, Helvetica'><b>$climate_name</b></td>
     </tr>
     <tr>
