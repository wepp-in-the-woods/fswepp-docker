...
<font face="Arial, Geneva, Helvetica">
  <table width="90%">
    <tr><td>
       ...
       <IMG src="http://',$wepphost,'/fswepp/images/e-drain.gif"
       align="left" border=1
       width=50 height=50
       alt="Return to X-DRAIN input screen"
...
#    else {
    ...
    print "<TR><TD><B>Climate station:</B></TD><TD>$stationnames, $region</TD>    <TD><BR></TD>\n";
    print "   <TD><B>Buffer length:</B></TD>";
    if ($units eq "m") {print "<TD>$bltextm[$bl] m</TD></TR>\n"};
    if ($units eq "ft") {print "<TD>$bltextf[$bl] ft</TD></TR>\n"};
    print "<TR><TD><B>Soil type:</B></TD><TD>$sttext[$st]</TD>    <TD><BR></TD>\n";
    print "    <TD><B>Buffer gradient:</B></TD><TD>";
        if ($bl == 0) {print 'n/a'}
        else {print $bstext[$bs]}
    print "</TD></TR>\n";
    print "<TR><TD></TD>    <TD></TD>    <TD></TD>\n";
    print "    <TD><B>Road width:</B></TD><TD>$width $units</TD></TR>\n";
    print "</TABLE>\n\n";
    print "<H2><B>Average annual sediment yield ";
    if ($raw<1) {
      if ($units eq "m") {print "(kg/m length)</B></H2>\n"};
      if ($units eq "ft") {print "(lb/ft length)</B></H2>\n"};
    }
    if ($raw>0) {
      if ($units eq "m") {print "(kg)</B></H2>\n"};
      if ($units eq "ft") {print "(lb)</B></H2>\n"};
    }
    print '<TABLE border="1" cellpadding="6">',"\n";

    print '<TR align=center><TH rowspan="2" bgcolor=85D2D2><B>Road <BR>Gradient</B></TH>
               <TH colspan="5" bgcolor=85D2D2><B>Cross drain spacing</B></TH></TR>
               <TR align=right>
';
    if ($units eq "m") {
