...
#   $lines = 1 if ($lines < 1);
   ...
   @results=`tail -$lines working/_$year/wrb.log`;
   @results=reverse(@results);
   $wc  = `wc working/_$year/wrb.log`;
   @words = split " ", $wc;
   $runs = @words[0];
...
print "<HTML>
 <HEAD>
  <TITLE>WEPP:Road Batch run log</TITLE>
 </head>
 <body onLoad='self.focus()'>
  <font face='tahoma, arial, sans serif'>
   <h3>$lines most recent of $runs WEPP:Road Batch runs (years * segments) for $year</h3>
   <pre>
 @results
   </pre>
  </font>
