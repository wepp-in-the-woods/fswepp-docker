...
#     read lines
#     split on tab $IP, $time, $climate, $latlon

while (<INPUT_FH>) {

     chomp;
     ($IP, $datetime,$climate,$latlong) = split "\t",$_,3;
#    $IP = @words[0];
#     $climate = @words[2];
#     $lat = @words[3];
#     $lon = @words[4];
...
#     if ($lon == '') {($lat, $lon) = split / /,$latlon}

#    print "$climate\t$lat\t$lon\n";

     $climates{$climate} = $latlon;

}

# print %climates;

    for my $climate (keys %climates) {
       print "$climates{$climate}\t$climate\n";
    }
