...
$bp=1;
   ...
   print "Content-type: text/html\n\n" if ($debug);
    ...
    $version = '2013.01.29';    # Start adding in breakpoint climate data capability
...
#    $unique='wepp' . time . '-' . $$;
     ...
     $soilPath     = 'data/';			# DEH 20110408
     $manPath      = 'datatahoebasin/';			# DEH 20110408
   }
...
#    @args = ("nice -20 ./wepp <$responseFile >$stoutFile 2>$sterFile");
     if ($platform eq "pc") {
       ...
       if ($weppversion ne '2010' && $weppversion ne '2000') {$weppversion = '2010'}	# DEH 20110414
...
';
      ...
      while (<WEPPFILE>) {
       ...
       $line +=1;
#       last if ($line > 100);
# ### temp ###       last if (/Management Section/);
      }
      close WEPPFILE;
      print '      filewindow.document.writeln("<\/pre><\/body><\/html>")
      filewindow.document.close()
...
#print CLOUT "$fullCliFile\n";     # ../rc/tahoe/fc/../rc/tahoe/fc/FallenLeaf_A2_srad.cli
 ...
 $line = <CLI>; print CLOUT $line; #         5.6     6.6     9.3    10.9    14.9    19.3    24.0    23.6    21.4    15.8    $
 $line = <CLI>; print CLOUT $line; #    Observed monthly ave min temperature (C)
 $line = <CLI>; print CLOUT $line; #        -7.7    -7.5    -6.0    -4.4    -1.3     1.2     4.5     4.3     2.1    -1.3    $
 $line = <CLI>; print CLOUT $line; #     Observed monthly ave solar radiation (Langleys/day)
 $line = <CLI>; print CLOUT $line; #       213.5   283.3   408.6   498.4   589.3   674.2   684.2   610.1   503.7   381.5   2$
 $line = <CLI>; print CLOUT $line; #    Observed monthly ave precipitation (mm)
 $line = <CLI>; print CLOUT $line; #       324.7   268.5   255.5   133.6    98.6    32.2     3.2     7.8    25.6    82.0   2$
 $line = <CLI>; print CLOUT $line; #     da       mo   year    prcp     dur     tp       ip    tmax    tmin     rad    w-vl $
 $line = <CLI>; print CLOUT $line; #                           (mm)     (h)                     (C)     (C)    (l/d)  (m/s) $
