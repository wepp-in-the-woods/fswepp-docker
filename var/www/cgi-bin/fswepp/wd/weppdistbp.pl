...
#  weppdistbp.pl -- input screen for Disturbed WEPP (no veg calibration needed)
...
$bp=1;
   ...
   $version = '2013.01.29';	# Start to allow breakpoint climate data
...
### get breakpoint climates

#   opendir CLIMDIR, '../rc/tahoe/fc/';                     
    opendir CLIMDIR, '../working/bp/';                  # DEH 08/29/2013
      @allfiles=readdir CLIMDIR;
    close CLIMDIR;

    for $f (@allfiles) {
      if (substr($f,-4) eq '.cli') {
        $f = '../working/bp/' . $f;
        open(M,$f) || goto bpskip;
          $lined = <M>; $lined = <M>;
          $station = <M>;
        close (M);
        $climate_file_bp[$num_cli] = substr($f, 0, -4);
        $clim_name = substr($station, index($station, ":")+2, 40);
        $clim_name =~ s/^\s*(.*?)\s*$/$1/;
        $climate_name_bp[$num_cli] = $clim_name;
        $num_cli += 1;
bpskip:
      }         # if (substr($f,-4) eq '.cli')
    }           # for $f (@allfiles) {
#  ####  index sort climate name  ####  #
   @ind = sort {$climate_name_bp[$a] cmp $climate_name_bp[$b]} 0..$#climate_name_bp;        # sort index
#  ####  copy sorted entries into climate name and file lists  ####  #
   for my $i (0..$#climate_name_bp) {
     $climate_name[$i] = $climate_name_bp[$ind[$i]];
     $climate_file[$i] = $climate_file_bp[$ind[$i]];
   }
...
#         $clim_name = '*' . substr($station, index($station, ":")+2, 20);
          $clim_name = substr($station, index($station, ":")+2, 20);
          $clim_name =~ s/^\s*(.*?)\s*$/$1/;
# chomp $clim_name;
...
para = para +'  Online at &lt;http://acwi.gov/sos/pubs/2ndJFIC/Contents/SP06_Dobre_03_29_10_paper.pdf&gt;. Accessed Nov., 2011.<br>'
...
para = para +'  Online at &lt;http://learningstore.uwex.edu/assets/pdfs/A3533.pdf&gt; Accessed Nov. 2011.'
...
print '<BODY bgcolor="white"
  ...
  <FORM name="weppdist" method="post" ACTION="wdbp.pl">
...
';
  ...
  $wc  = `wc ../working/wd2.log`;
  @words = split " ", $wc;
  $runs = @words[0];

print "  $remote_host &ndash; $remote_address ($user_really) personality '<b>$me</b>'<br>
  <b>$runs</b> Disturbed WEPP 2.0 Model runs since May 1, 2012
<br>
 </body>
</html>
";
