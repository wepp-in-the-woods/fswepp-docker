...
# David Hall, USDA Forest Service, Rocky Mountain Research Station
   ...
   $version='2012.03.06';	# try to combat time-out error some users are experiencing
...
# ======================  CUSTOM CLIMATE  ======================
   ...
   if (lc($action) =~ /custom/) {
     $wepproadbat = "http://" . $wepphost . "/cgi-bin/fswepp/wr/wepproadbat.pl"; 
     if ($platform eq "pc") {
...
############# TIME-OUT COUNTER USER FEEDBACK START ###########  2012
#
#

# 1... make socket filehandle hot (how?)	# perl.plover.com/FAQs/Buffering.html
# 	The usual complaint:
#	 "I opened the connection all right, and I got the greeting from the server, but it isn't responding
#	 to my client's commands!"
#	 And by now you know the reason why:
#	 The client's output to the network socket is being buffered, and Perl is waiting to send the data over the network
#	 until there's a whole bufferful.
#	 The server hasn't responded because it hasn't received anything to respond to.
#	Solution: Make the socket filehandle hot.
#	 Another solution: Use the IO::Socket module; recent versions (since version 1.18) make sockets hot by default. 

#   select((select(STDOUT), $|=1)[0]);			# perl.plover.com/FAQs/Buffering.html

    $|=1;
    binmode( STDOUT, ":unix" );				# http://perldoc.perl.org/perlfaq5.html

# 2... change "print" to "syswrite" (no buffering)	# perl.plover.com/FAQs/Buffering.html

#  Other Directions
#	If for some reason you want to avoid the buffering entirely, you can use Perl's sysread and syswrite operators.
#	 These don't use buffering at all. That makes them slow, but they are often appropriate for tasks like
#	 network communications where you don't want buffering anyway.
#	 All Perl's other I/O functions, including write, print, read, <FILEHANDLE>, and getc, are buffered.
#	 If you do both buffered and unbuffered I/O on the same filehandle, you're likely to confuse yourself, so beware. 

# 3... flush after the headers				# www.stevesouders.com/blog/2009/05/18/flushing-the-document-early/
#	 Flushing is when the server sends the initial part of the HTML document to the client before the entire response
#	 is ready. All major browsers start parsing the partial response.
#	 When done correctly, flushing results in a page that loads and feels faster.
#	 The key is choosing the right point at which to flush the partial HTML document response.
#	 The flush should occur before the expensive parts of the back end work, such as database queries
#	 and web service calls.
#	 But the flush should occur after the initial response has enough content to keep the browser busy.
#	 The part of the HTML document that is flushed should contain some resources as well as some visible content.
#	 If resources (e.g., stylesheets, external scripts, and images) are included, the browser gets an early start
#	 on its download work.
#	 If some visible content is included, the user receives feedback sooner that the page is loading.
#	Most HTML templating languages, including PHP, Perl, Python, and Ruby, contain a "flush" function.
#	 Getting flushing to work can be tricky. Problems arise when output is buffered, ...

#  flush();

# 4... close/reopen STDOUT?
...
#
#
############# TIME-OUT COUNTER USER FEEDBACK END ###########  2012
...
';
   ...
   <table width="90%">
    ...
    <td>
<!--
       <A HREF="/fswepp/docs/wroadbimg.html#wrout">
...
#######################   check continue
     ...
     print "
     <FORM name=\"wrbat\" method=post ACTION=\"/cgi-bin/fswepp/wr/wrbatnoto.pl\">
    ...
    <tr>
     <td>
      <font face="Arial, Geneva, Helvetica, sans serif" size=2>
       ...
       <a href="http://forest.moscowfsl.wsu.edu/people/engr/dehall.html">David Hall</a>
       and 
       Darrell Anderson;
       Project leader
       <a href="http://forest.moscowfsl.wsu.edu/people/engr/welliot.html">Bill Elliot</a>
       <br>
...
### record model runs ###
     ...
     $ditlogfile = '>>../working/wrb/' . $thisweek;
...
### record road segment count into WEPP:ROad logs###
     ...
     $ditlogfile = '>>../working/wr/' . $thisweek;
     open MYLOG,$ditlogfile;
       flock MYLOG,2;                   # 2005.02.09 DEH
       print MYLOG '.' x $count;        # 2005.02.10 DEH
     close MYLOG;
