...
";
     print '
 <BODY background="http://',$wepphost,'/fswepp/images/note.gif" link="#ff0000">
   ...
   <table width="90%">
    <tr>
     <td>
      ...
      <img src="http://',$wepphost,'/fswepp/images/roadb.gif"
        align=left border=1
        width=50 height=50
    ...
    <td align=center>
     <hr>
     <H3>WEPP:Road Batch results<br>
      <img src="/fswepp/images/underc.gif">
     </H3>
     <hr>
    <td>
<!--
       <A HREF="http://',$wepphost,'/fswepp/docs/wroadbimg.html#wrout">
...
##    next if ($head eq $prev_head);
   ...
   open SPREAD, '<../working/'.$inputfilename;
   while (<SPREAD>) {
     last if ($count > $count_limit);
     $head=$_;
     $count+=1;
...
#######################   check continue
     print "
     <FORM name=\"wrbat\" method=post ACTION=\"http://$wepphost/cgi-bin/fswepp/wr/wrbatf.pl\">
      ...
      <input type=\"hidden\" name=\"projectdescription\" value=\"$project_title\">
       ...
       <input type=\"text\" name=\"inputfile\" size=50 value=$inputfilename>
      </a>
      <br>
...
#  $project_title='for testing only';
      ...
      open LOG, ">" . $logFile;
      print LOG "$project_title\tProject
$climate_name
$STx\tSoil texture
$units\tUnits
$years\tYears
";
      if ($units == 'ft') {
        print LOG "Precip (in)\tRecord\t\tRoad Design\tSurface Traffic\tRoad grad (pct)\tRoad l (ft)\tRoad w (ft)\tFill gr (pct)\tFill l (ft)\tBuffer gr (pct)\tBuffer l (ft)\tRock (pct)\tR runoff (in)\tS runoff (in)\tRoad prism erosion (lb)\tBuffer sediment (lb)\tComment\n";
      }
      else {
        print LOG "Precip ()\tRecord\t\tRoad Design\tSurface Traffic\tRoad grad (pct)\tRoad l ()\tRoad w ()\tFill gr (pct)\tFill l ()\tBuffer gr (pct)\tBuffer l ()\tRock (pct)\tR runoff ()\tS runoff ()\tRoad prism erosion ()\tBuffer sediment ()\tComment\n";
      }
      close LOG;

# =======================  Run WEPP  =========================

 print "<font size=-1>",&whatdate,"</font>";
...
#$comments
#";
      print LOG "$precipf\t$record\t$bang\t$designw\t$surface $traffic\t$URS\t$URL\t$URW\t$UFS\t$UFL\t$UBS\t$UBL\t$UBR\t$rrof\t$srof\t$syraf\t$sypaf\t$comments\n";
      close LOG;
}
...
#          WEPP's error message follows:  \n<p><hr><p>\n";
         ...
         open LOG, ">>" . $logFile;
           print LOG "$precipf\t$record\t$bang\t$designw\t$surface $traffic\t$URS\t$URL\t$URW\t$UFS\t$UFL\t$UBS\t$UBL\t$rf\t\t\t\t\t\t$comments\n";
         close LOG;
         $bang='';
       }
...
#       $minURL = 3;   $maxURL = 1000;
       $minURL = 0.2;   $maxURL = 9100;
#       $minURS = 0.3; $maxURS = 40;
       $minURS = 0.0; $maxURS = 160;
       $minURW = 1;   $maxURW = 300;
#       $minUFL = 1;   $maxUFL = 300;
       $minUFL = 0.00;   $maxUFL = 300;
...
#  if ($surface eq 'b') {$min=0.1; $max=100}
  if ($surface eq 'r') {$min=0.00; $max=160}
  if ($surface eq 'f') {$min=0.00; $max=150}
  if ($surface eq 'b') {$min=0.00; $max=100}
...
#  if ($surface eq 'r' && $units eq 'ft') {$min=0;   $max=6000} #  if ($surface eq 'r' && $units eq 'ft') {$min=3;   $max=1000}
#  if ($surface eq 'f' && $units eq 'm')  {$min=0.3; $max=100}
#  if ($surface eq 'f' && $units eq 'ft') {$min=0;   $max=6000}
#  if ($surface eq 'b' && $units eq 'm')  {$min=0.3; $max=300}
#  if ($surface eq 'b' && $units eq 'ft') {$min=0;   $max=6000} #  if ($surface eq 'b' && $units eq 'ft') {$min=1;   $max=1000}
#  if ($surface eq 'w' && $units eq 'm')  {$min=0.3; $max=100}
#  if ($surface eq 'w' && $units eq 'ft') {$min=1;   $max=300}
  if ($surface eq 'r' && $units eq 'm')  {$min=1;   $max=300}
  if ($surface eq 'r' && $units eq 'ft') {$min=0.02;   $max=9100} #  if ($surface eq 'r' && $units eq 'ft') {$min=3;   $max=1000}
  if ($surface eq 'f' && $units eq 'm')  {$min=0.3; $max=100}
  if ($surface eq 'f' && $units eq 'ft') {$min=0.00;   $max=6000}
  if ($surface eq 'b' && $units eq 'm')  {$min=0.3; $max=300}
  if ($surface eq 'b' && $units eq 'ft') {$min=0.02;   $max=6000} #  if ($surface eq 'b' && $units eq 'ft') {$min=1;   $max=1000}
  if ($surface eq 'w' && $units eq 'm')  {$min=0.3; $max=100}
  if ($surface eq 'w' && $units eq 'ft') {$min=0.02;   $max=6000}
