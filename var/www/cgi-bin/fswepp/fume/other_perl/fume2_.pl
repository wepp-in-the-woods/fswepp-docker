...
#  Modified by DEH from wd1.pl by Elena V. 2004.04.10
  ...
  $version = '2004.10.01';
# $version = "2004.09.23";		# Beta release for Missoula presentation
  $debug=0;

# Reads user input from fume.pl,
#   runs Disturbed WEPP (4 runs)
...
#	move "Content-type: text/html\n\n"; down (so not before describe-climate custom-climate)
# 2004.10.01 DEH fix ofe2_pcover (was using ofe1)
# 2004.07.29 Add form link to WEPP:Road for each run;
#		add link to WEPP:Road and Disturbed WEPP input screens
#		change from "# Disturbed WEPP runs" to "# WEPP FuMe runs"
#		(counter not installed 'cuz it's not released)
# 2004.07.23 check min/max of 4 returned road sediment options, not hardwire
#	round answers just before printing, not during calculations
#	check for 0 road density; don't do WEPP:Road if so
#	customize a bit the discussion paragraph
# 2004.07.21 Move equation report out of tables
# David Hall, USDA Forest Service, Rocky Mountain Research Station, Moscow
# Elena Velasquez, USFS Missoula
...
#####  Read user input parameters  #####
   ...
   $wildfire_cycle=$parameters{'wildfire_cycle'}+0;	# 2004.09.16
   $rx_fire_cycle=$parameters{'rx_fire_cycle'}+0;		# 2004.09.16
   $thinning_cycle=$parameters{'thinning_cycle'}+0;	# 2004.09.16
...
#  $hillslope_length=$parameters{'hillslope_length'}+0;
   $hillslope_length=$parameters{'totall'}+0;
...
#  $buffer_length=$parameters{'buffer_length'}+0;
   $buffer_length=$parameters{'buffl'}+0;
...
#   $wildfirecycle=$parameters{'wildfire_cycle'}; #elena	# 2004.09.16
#   $fuelmangcycle=$parameters{'fuelmang_cycle'}; #elena	# 2004.09.16
...
# initialization for inputs array 
   @runheadd=('Undisturbed forest','Thinned forest','Prescribed burn','Wildfire');
   @intreat1=('tree20','tree5','low','high');
   @intreat2=('tree20','tree20','tree20','high');
   @inofe1_pcover=(100,85,85,30);		# 2004.09.15
   @inofe2_pcover=(100,100,100,30);		# 2004.09.15
...
#     $weppdiro = "http://" . $wepphost . "/Scripts/fswepp/wd/weppdiro.pl";
     $fume = "http://" . $wepphost . "/cgi-bin/fswepp/fume/fume.pl";		# 2004.09.21 DEH
...
#       exec "perl c:\\Inetpub\\Scripts\\fswepp\\rc\\rockclim.pl -server -i$me -u$units $fume"
   ...
   if (lc($action ne 'Run WEPP') && lc($achtung) =~ /describe climate/) {
#     $weppdiro = "http://" . $wepphost . "/Scripts/fswepp/wd/weppdiro.pl";
     $fume = "http://" . $wepphost . "/cgi-bin/fswepp/fume/fume.pl";
# print "<p>I am inside describe and wepphost is $wepphost <br> cl is $CL<br> weppdiro is $fume";#elena
     if ($platform eq "pc") {
#elena       exec "perl ../rc/descpar.pl $CL $weppdiro"
       exec "perl c:\\Inetpub\\Scripts\\fswepp\\rc\\descpar.pl $CL $fume" #elena
     }
     else {
       exec "../rc/descpar.pl $CL $fume"
     }
     die
   }		# /describe climate/
...
#     else {$working = 'c:\\Inetpub\\Scripts\\fswepp\\working'} #elena
     $responseFile = "$working\\fume.in";
     $outputFile   = "$working\\fume.out";
     $stoutFile    = "$working\\fume.sto";
     $sterFile     = "$working\\fume.err";
     $slopeFile    = "$working\\fume.slp";
     $soilFile     = "$working\\fume.sol";
     $cropFile     = "$working\\fume.crp";
     $climateFile  = "$CL" . '.cli';
     $manFile      = "$working\\fume.man";
     $soilPath     = 'data\\';
     $manPath      = 'data\\';
     $newSoilFile  = "$working\\fume.sol";	# WEPP:Road
     $tempFile     = "$working\\temp";		# 2004.09.15
   }
   else {
     $unique='wepp' . '-' . $$;
     $working = '../working';
     ...
     $tempFile     = "$working/$unique" . ".temp";	# 2004.09.15 # 2004.11.23
   }
 ...
 <HEAD>
  ...
  <script language="Javascript">
    ...
    function popuphistory() {
     ...
     popupwindow.document.writeln('  <title>WEPP FuMe version history</title>')
     popupwindow.document.writeln(' </head>')
     popupwindow.document.writeln(' <body bgcolor=white>')
     popupwindow.document.writeln('  <font face="trebuchet, tahoma, arial, helvetica, sans serif">')
     popupwindow.document.writeln('  <center>')
     popupwindow.document.writeln('   <h3>WEPP FuMe Version History</h3>')
     ...
     popupwindow.document.writeln('    </tr>')
     popupwindow.document.writeln('    <tr>')
     popupwindow.document.writeln('     <th valign=top bgcolor=lightblue>2004.09.23</th>')
     popupwindow.document.writeln('     <td>Beta release for Missoula presentation</td>')
     popupwindow.document.writeln('    </tr>')
   ...
   <blockquote> 
    <table width=100% border=0>
     <tr>
      ...
      <td align=center>
       <hr>
       <h3>WEPP <font color="red">FuMe</font><br>
           <font color="red">Fu</font>el
           <font color="red">M</font>anagement
           <font color="red">E</font>rosion Analysis Results</h3>
       <hr>
      <td>
       <A HREF="http://',$wepphost,'/fswepp/docs/fume/0409_Draft_Watershed_Chapter_.htm" target="docs">
       <IMG src="http://',$wepphost,'/fswepp/images/epage.gif"
        align="right" alt="FuMe documentation" border=0></a>
      </td>
     </tr>
    </table>
';
...
#######################  do DISTURBED WEPP runs #########################
   ...
   print "   <br>Running <b>Disturbed WEPP</b> for \n";
   ...
   <table width=100% border=0>
    ...
    <tr>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Upper treatment</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Lower treatment</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Upper cover<br>(%)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Lower cover<br>(%)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Upper length<br>(ft)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Lower length<br>(ft)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Upper rock<br>(%)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Lower rock<br>(%)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Upper top gradient<br>(%)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Upper mid gradient<br>(%)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Lower mid gradient<br>(%)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Lower toe gradient<br>(%)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1><a href="/fswepp/docs/fume/equations.html" target="doc">Sediment yield</a><br>(t/mi<sup>2</sup>/y)</th>
     <th bgcolor="lightgreen"><font face="tahoma" size=1>Condition</th>
    </tr>
';

#  moved this out of the loop (doing directly crashes the run...)
...
########################### elena new loop #############################
  ...
  for($i = 0; $i < 4; $i++){
   ...
   $ofe2_pcover=$inofe2_pcover[$i]; #kova      # 2004.10.01 DEH was $inofe1_pcover...
...
#   $wildfire=($out_asypa[3]*640)/$wildfire_cycle; 
   ...
   <a
    onMouseOver=\"window.status='from WEPP'\"
    onMouseOut=\"window.status=''\">
    $syp</a> kg m<sup>-1</sup> y<sup>-1</sup> *
   <a
    onMouseOver=\"window.status='unit conversion'\"
    onMouseOut=\"window.status=''\">
    10</a> (kg m<sup>-1</sup> -&gt; t ha<sup>-1</sup>) *
   <a
    onMouseOver=\"window.status='unit conversion'\"
    onMouseOut=\"window.status=''\">
    0.445</a> (t ha<sup>-1</sup> -&gt; t a<sup>-1</sup>) *
   <a
    onMouseOver=\"window.status='unit conversion'\"
    onMouseOut=\"window.status=''\">
    640</a> a mi<sup>-1</sup> *
   <a
    onMouseOver=\"window.status='slope length'\"
    onMouseOut=\"window.status=''\">
    $slope_lengthf</a> m length
";
...
print TEMP "
     <td bgcolor='lightgreen' align='right'>
      <font face='tahoma' size=1>
       <a 
        onMouseOver=\"window.status='$syp kg/y/m width, $slope_lengthf m slope length'\"
        onMouseOut=\"window.status=''\">
       $z_out_asypa</a><br>
      </font>
     </td>
...
";
  ...
  } ################# i loop for Disturbed runs (0 to 3)
...
# die;
  ...
  if ($roaddensity < 0.001 || $roaddensity eq '0') {
    $doroad=0;
    $no_road_min=0; $no_road_max=0; $no_road_low=0; $no_road_high=0;
    $tr_road_min=0; $tr_road_max=0; $tr_road_low=0; $tr_road_high=0;
  }
  if ($doroad) {

   print "   Running <b>WEPP:Road</b> for \n";
...
#  $UBL=50;           # User's buffer length
   ...
   $UFS=$ofe1_mid_slope*2; # If fill gradient two times hill gradient WJE 04.07.08
    ...
    <tr>
     <th bgcolor="lightgreen">
      ...
      (<a
          ...
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'ib\'</a>,
       <a
          onMouseOver="window.status=\'Insloped road with vegetated or rocked ditch\'"
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'iv\'</a>,<br>
       <a
          onMouseOver="window.status=\'Outsloped rutted road\'"
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'or\'</a>,
       <a
          onMouseOver="window.status=\'Outsloped unrutted road\'"
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'ou\'</a>)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Road surface<br>
      (<a onMouseOver="window.status=\'Native surface\'"
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'n\'</a>,
       <a onMouseOver="window.status=\'Gravel surface\'"
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'g\'</a>,
       <a onMouseOver="window.status=\'Paved surface\'"
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'p\'</a>)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Traffic level<br>
      (<a onMouseOver="window.status=\'High traffic\'"
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'h\'</a>,
       <a onMouseOver="window.status=\'Low traffic\'"
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">\'l\'</a>,
     ...
     <th bgcolor="lightgreen">
       <font face="tahoma" size=1>
        <a
          ...
          onMouseOut="window.status=\'Forest Service WEPP FuMe\'">Road<br>gradient</a><br>(%)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Road<br>length<br>(ft)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Road<br>width<br>(ft)
     ...
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Fill<br>length<br>(ft)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Buffer<br>gradient<br>(%)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Buffer<br>length<br>(ft)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Rock<br>fragment<br>(%)
     ...
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       <a href="/fswepp/docs/fume/equations.html#road" target="doc">Sediment yield<br>from road</a><br>(t/mi<sup>2</sup>/y)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       <a href="/fswepp/docs/fume/equations.html#road" target="doc">Sediment yield<br>from buffer</a><br>(t mi<sup>-2</sup> y<sup>-1</sup>)
     </th>
     <th bgcolor="lightgreen">
      <font face="tahoma" size=1>
       Condition
 ...
 $NUMBERS .= "<br><br>
  ...
  from road:
   ...
   <a
    onMouseOver=\"window.status='from WEPP'\"
    onMouseOut=\"window.status=''\">
    $syr</a> kg m<sup>-2</sup> y<sup>-1</sup> *
   <a
    onMouseOver=\"window.status='unit conversion'\"
    onMouseOut=\"window.status=''\">
    2.2046</a> lb kg<sup>-1</sup> *
   <a
    onMouseOver=\"window.status='unit conversion'\"
    onMouseOut=\"window.status=''\">
    0.0088</a> *
   <a
    onMouseOver=\"window.status='road density'\"
    onMouseOut=\"window.status=''\">
    $roaddensity</a> mi mi<sup>-2</sup> *
   <a
    onMouseOver=\"window.status='effective road length'\"
    onMouseOut=\"window.status=''\">
    $eff_rdlenf</a> m effective road length *
   <a
    onMouseOver=\"window.status='road width'\"
    onMouseOut=\"window.status=''\">
    $WeppRoadWidth</a> m road width
  <br>
  from buffer:
   ...
   <a
    onMouseOver=\"window.status='from WEPP'\"
    onMouseOut=\"window.status=''\">
   $syp kg m<sup>-1</sup> y<sup>-1</sup> *
   <a
    onMouseOver=\"window.status='unit conversion'\"
    onMouseOut=\"window.status=''\">
   2.2046 lb kg<sup>-1</sup> *
   <a
    onMouseOver=\"window.status='unit conversion'\"
    onMouseOut=\"window.status=''\">
   0.0088 *
   ...
   print TEMP "
    <tr>
     ...
     <td bgcolor='lightgreen' align='right'>
      <font face='tahoma' size=1>
       <a 
        onMouseOver=\"window.status='$syr kg/y/m2, $WeppRoadWidth m road width'\"
        onMouseOut=\"window.status=''\">
       @z_syraf[$i]</a>
      </font>
     </td>
     <td bgcolor='lightgreen' align='right'>
      <font face='tahoma' size=1>
       <a 
        onMouseOver=\"window.status='$syp kg/y/m width, $WeppRoadWidth m road width, $eff_rdlenf m effective road length'\"
        onMouseOut=\"window.status=''\">
       @z_sypaf[$i]</a>
      </font>
     </td>
...
#  $undiraw=@out_asypa[0]*640;
   ...
   $undisturbe=$out_asypa[0]*640; #elena
   $thinning=sprintf '%.1f',($out_asypa[1]*640)/$thinning_cycle; #elena # 2004.09.16
   $prescribe=sprintf '%.1f',($out_asypa[2]*640)/$rx_fire_cycle; #elena # 2004.09.16
   $wildfire=sprintf '%.1f',($out_asypa[3]*640)/$wildfire_cycle; #elena # 2004.09.16
   $notbackground=$undisturbe+$wildfire; #elena
   $witbackground=$thinning+$prescribe; #elena
   $withbperofnotb= sprintf('%.1f' , ((100*$witbackground)/$notbackground)); #elena
...
print "
     <center>
      <h4><b>Output summary</b> based on $years2sim years of possible weather</h4>
	...
	<th bgcolor='lightblue'>
         <font face='Trebuchet, Tahoma, Arial, Geneva, Helvetica'>
          Erosion in year of disturbance<br>(ton mi<sup>-2</sup>)
         </font>
	</th>
	<th bgcolor='lightblue'>
         <font face='Trebuchet, Tahoma, Arial, Geneva, Helvetica'>
	...
	<th bgcolor='lightblue'>
         <font face='Trebuchet, Tahoma, Arial, Geneva, Helvetica'>
          \"Average\" annual sedimentation<br>
...
# @@@@@@@@@@@@@@@@@@@@
   ...
   $thin_over_back = sprintf '%.0f', 100 * $thinning / $undiwild if ($undiwild>0.001);
   ...
   $presraw_over_wildfire_cycle = sprintf '%.2f', $presraw / $wildfire_cycle if ($wildfire_cycle>0.01);
...
print "
    ...
    From these runs, several key watershed sedimentation values can be determined.
    <br><br>
    ...
    In the absence of roads, the background sedimentation rate is from wildfire and erosion from an undisturbed forest.
    This value is the sum of lines 1 and 2, or
	<font color=green>
        $undisturbe + $wildfire = $notbackground
	</font>
    ton&nbsp;mi<sup>-2</sup>&nbsp;y<sup>-1</sup>.
    If the existing low traffic road network is included in the background sediment rate,
    ...
    high traffic level to support the traffic associated with an ongoing thinning operation in the watershed. 
    The total sediment yield from the watershed will then be the background value plus that from thinning and
    high traffic roads for a total of
	<font color=green>
        $notbackground +
        $thinback +
        ($tr_road_min to
	...
	<font color=green>
        $tr_thin_low_over_back to
        $tr_thin_high_over_back
 	</font>
    ...
    or that thinning will lead to a less severe wildfire, and the prescribed fire sedimentation rate from
    line 3 can be substituted for the wildfire erosion rate in line 2, reducing the background calculation.
    <br><br>
    ...
    plus the low traffic road rate and the average erosion from prescribed fire, or
	<font color=green>
        $notbackground +
        $prescribe +
        ($no_road_min to
    ...
    (line 1 of the analysis) for the analysis.
    Alternatively, the impact of the prescribed fire program may be to reduce the intensity of the wildfire,
    in which case, the sedimentation associated with prescribed fire can be substituted for the wildfire prediction
    for the analysis.
    <br><br>
    ...
    the thinning rate, the prescribed fire rate, and the high traffic road rate.
    ...
    then the prescribed fire sedimentation value of
	<font color=green>
        $presraw
	</font>
    ton&nbsp;mi<sup>-2</sup>&nbsp;y<sup>-1</sup> can be substituted for the wildfire erosion rate once every
	<font color=green>
        $wildfire_cycle
	</font>
    years to give an average value of
	<font color=green>
        $presraw_over_wildfire_cycle
	</font>
    ton&nbsp;mi<sup>-2</sup>&nbsp;y<sup>-1</sup>.
    ...
    ton&nbsp;mi<sup>-2</sup>&nbsp;y<sup>-1</sup>,
       a decrease of
	<font color=green>
        $rx_wild_low_over_back to
        $rx_wild_high_over_back
	</font>
...
";
    ...
    Roads with buffers greater than that specified will generate less sediment.
    ...
    Results from each hillslope can be copied and pasted into a word processor to serve as a log for a series of runs.
    <br><br>
    &nbsp;&nbsp;&nbsp;<b><em>Further Information.</em></b>
...
Interface v. 
 ...
 (for review only) by David Hall and Elena Velasquez,
...
#  <b>$runs</b> WEPP FuMe runs since January 1<br>
print "
Not yet released.<br>
";
...
# $wroadout{'sedroad'}=[@outsyraf];
# $wroadout{'sedbuff'}=[@outsypaf];

#kova print "<p> this is outsyraf: @{$wroadout{'sedroad'}}";
# return %wroadout;
  return ($syraf, $sypaf);
...
####
    if ($debug) {print "[CreateCligenFilewr]<br>
      Arguments:    $args<br>
      ClimatePar:   $climatePar<br>
      ClimateFile:  $climateFile<br>
      OutputFile:   $outfile<br>
