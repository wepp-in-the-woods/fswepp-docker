...
## BEGIN HISTORY ##############################
# WEPP FuME Results history
   $version = '2005.11.02';	# Report climate parameter modifications (if any)
#  $version = '2005.09.12';	# Fix parsing of sediment yield on Disturbed WEPP run results -- lost data for very wet climates
#  $version = '2005.04.29';	# Limit fillslope (2*user slope) to 150% max, run without warning
#  $version = '2005.04.28';	# Add mass failure warning for hillslope gradients greater than 50%
#  $version = '2005.03.18';	# Work on narrative
#  $version = '2005.03.17';	# Slight formatting changes
#  $version = '2005.03.16';	# Work on narritive calculations
#  $version = '2005.03.10';	# Reconnect WEPP:Road runs behind Conditions link
#  $version = '2005.02.09';	# Add live history popup
#  $version = '2005.02.08';	# Make changes noted by WJE 2/7/05 -- wording in [Summary of Analysis]
#  $version = '2004.12.09';	# Record run in log and return count
#  $version = '2004.12.09';     # Correct heading for sed yield for Disturbed WEPP table (not per year)
#! $version = '2004.12.07';	# Change 'low traffic' and 'high traffic' to '...access'
#! $version = '2004.12.07';     # Fix Thinning 'In order to...' value in narrative
#  $version = '2004.12.07';     # Fix percentage calculations (some still off?)
#! $version = '2004.11.30';	# Set WEPP output variables to null before run (so no drift-over)
#! $version = '2004.11.23';	# Move 'Content-type' to below calls out to Rock:Clime etc., change font sizes
#  $version = '2004.11.19';	# Add a variety of Disturbed WEPP runs for other conditions
#! $version = '2004.10.01';
...
# 2004.11.23 DEH move "Content-type: text/html\n\n"; down (so not before describe-climate custom-climate)
#		 reduce font size (-2) for progress report Disturbed and Road and (-1) for Summary of Analysis
...
# initialization for inputs array 
   @runheadd=('Undisturbed forest','Thinned forest','Prescribed burn','Wildfire','Lower thinning','Higher Rx fire','Lower Rx fire','Moderate wildfire','Low wildfire');
   @intreat1=('tree20','tree5','low','high','tree5','low','low','low','low');
   @intreat2=('tree20','tree20','tree20','high','tree20','low','tree20','low','low');
...
#       exec "perl c:\\Inetpub\\Scripts\\fswepp\\rc\\rockclim.pl -server -i$me -u$units $fume"
   ...
   if (lc($action) ne 'run wepp fume' && lc($achtung) =~ /describe climate/) {	# 2004.11.19 DEH
     $weppdiro = "http://" . $wepphost . "/Scripts/fswepp/wd/weppdiro.pl";
...
# 2005.04.28 DEH
  ...
  if ($ofe1_top_slope>50 || $ofe1_mid_slope>50 || $ofe2_bot_slope>50) {
    ...
    $grad_warn='<font color=red size=-1><sup>*</sup> Hillslopes with greater than 50% gradient may be prone to mass failure.</font>';
  }
...
#  for($i = 0; $i < 4; $i++){
  for($i = 0; $i < $#intreat1+1; $i++){		# 2004.11.19 DEH
   ...
   $ofe1_length=$user_ofe1_length; #elena	# 2004.11.30 DEH move out -- bombed out low/mod wildfire...
...
#          if ($syp eq "") {$syp = substr $_,10,9}	# 2005.09.12 DEH
           if ($syp eq "") {$syp = substr $_,1,18}	# 2005.09.12 DEH; could change to regular pattern match
							# eg if (/*([0-9]+)/) {$syp = $1}
           $_ = <weppout>;
           $_ = <weppout>;
           last;
 ...
 $NUMBERS .= "<br><br>
  ...
  from buffer:
   ...
   $WeppRoadWidth</a> m road width
    ";
# $j=$i+4;
$j=$i+9;		# 2005.03.10 DEH (accomodate added Disturbed WEPP runs in previous table!
   print TEMP "
    <tr>
     <td bgcolor='lightblue'>
...
#   $rx_over_back = sprintf '%.0f', 100 * $prescribe / $undiwild if ($undiwild>0.001);
   ...
   $no_low_over_back = sprintf '%.0f', 100 * ($no_road_low-$undiwild) / $undiwild if ($undiwild>0.001);
   $no_high_over_back = sprintf '%.0f', 100 * ($no_road_high-$undiwild) / $undiwild if ($undiwild>0.001);
   $tr_low_over_back = sprintf '%.0f', 100 * ($tr_road_low-$undiwild) / $undiwild if ($undiwild>0.001);
   $tr_high_over_back = sprintf '%.0f', 100 * ($tr_road_high-$undiwild) / $undiwild if ($undiwild>0.001);
   $tr_thin_low_over_back = sprintf '%.0f', 100 * ($tr_thin_low-$undiwild) / $undiwild if ($undiwild>0.001);
   $tr_thin_high_over_back = sprintf '%.0f', 100 * ($tr_thin_high-$undiwild) / $undiwild if ($undiwild>0.001);
   $tr_thin_low1_over_back = sprintf '%.0f', 100 * ($tr_thin_low1-$undiwild) / $undiwild if ($undiwild>0.001);
   $tr_thin_high1_over_back = sprintf '%.0f', 100 * ($tr_thin_high1-$undiwild) / $undiwild if ($undiwild>0.001);
...
#   $rx_high_over_back = sprintf '%.0f', 100 * $rx_high / $undiwild if ($undiwild>0.001);
   ...
   $rx_low_over_back = sprintf '%.0f', 100 * ($rx_low-$undiwild) / $undiwild if ($undiwild>0.001);
   $rx_high_over_back = sprintf '%.0f', 100 * ($rx_high-$undiwild) / $undiwild if ($undiwild>0.001);

   $presraw_over_wildfire_cycle = sprintf '%.2f', $presraw / $wildfire_cycle if ($wildfire_cycle>0.01);
   $wildmodraw_over_wildfire_cycle = sprintf '%.2f', $wildmodraw / $wildfire_cycle if ($wildfire_cycle>0.01);
...
#   $rx_wild_high_over_back = sprintf '%.0f', 100 * $rx_wild_high / $undiwild if ($undiwild>0.001);
   ...
   $rx_wild_low_over_back = sprintf '%.0f', -100 * ($rx_wild_low-$undiwild) / $undiwild if ($undiwild>0.001);
   $rx_wild_high_over_back = sprintf '%.0f', -100 * ($rx_wild_high-$undiwild) / $undiwild if ($undiwild>0.001);
...
";
	...
	<font color=green>
";
    printf '%.0f', -100*($tempmax-$no_road_high) / $no_road_high if ($no_road_high > 0.01);
    print "     \n    to\n     ";
    printf '%.0f', -100*($tempmin-$no_road_low) / $no_road_low if ($no_road_low > 0.01);
    print "
	</font>
    percent compared to background with roads or
        <font color=green>
";
    printf '%.0f', -100*($tempmax-$notbackground) / $notbackground if ($notbackground > 0.01);
    print "     \n    to\n     ";
    printf '%.0f', -100*($tempmin-$notbackground) / $notbackground if ($notbackground > 0.01);
    print "
        </font>
...
print "
    <br>
    <b>$runs</b> WEPP FuME runs since release, December 2004<br>
   </font>
  </font>
 </body>
</html>
...
# Record run in log
     ...
     open WFLOG, ">>../working/wf.log";
       flock WFLOG,2;			# 2005.02.09 DEH
       print WFLOG "$host\t";
       printf WFLOG '%0.2d:%0.2d ', $hour, $min;
       print WFLOG $ampm[$ampmi],"  ",$days[$wday]," ",$months[$mon]," ",$mday,", ",$year+1900, "\t";
       print WFLOG $climate_name,"\n";
     close WFLOG;
...
#     print "$thisday\t $thisweek\n";
     $ditlogfile = '>>../working/wf/' . $thisweek;
...
#    if ($UFS < $minUFS or $UFS > $maxUFS) {$rc=$rc-1; print "Fill gradient must be between $minUFS and $maxUFS %<BR>\n"}
     if ($UFS < $minUFS) {$rc=$rc-1; print "Fill gradient must be greater than $minUFS %<BR>\n"}
     if ($UFS > $maxUFS) {
       $UFS = $maxUFS
     }
# 2005.04.29 DEH end
