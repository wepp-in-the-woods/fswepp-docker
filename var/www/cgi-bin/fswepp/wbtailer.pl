...
#   $lines = 1 if ($lines < 1);
   ...
   $year = int ($year+0);
   $year = 2014 if ($year eq 0);

   $lines=200;

   @results=`tail -$lines working/_$year/wb.log`;
   @results=reverse(@results);
   $wc  = `wc working/_$year/wb.log`;
   @words = split " ", $wc;
   $runs = @words[0];
...
print "<HTML>
 <HEAD>
  <TITLE>BioMass run log</TITLE>
 </head>
 <body onLoad='self.focus()'>
  <font face='tahoma, arial, sans serif'>
   <h3>$lines most recent of $runs BioMass runs</h3>
   <pre>
 @results
   </pre>
  </font>
