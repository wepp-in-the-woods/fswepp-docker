...
#         $syp+=0;
  ...
  <table cellspacing=8>
   ...
   <tr><td>$precipf<td>$precipunits<td>precipitation from
       <td align=right>$storms<td>storms
   <tr><td>$rrof<td>$precipunits<td>runoff from rainfall from
       <td align=right>$rainevents<td>events
   <tr><td>$srof<td>$precipunits<td>runoff from snowmelt or winter rainstorm from 
       <td align=right>$snowevents<td>events
   <tr><td>$syraf<td>$sedunits<td>upland erosion rate (yyy kg/m<sup>2</sup>)
   <tr><td>$sypaf<td>$sedunits<td>sediment leaving profile (xxx kg/m width)
</table>
<hr width=50%>
";
...
# select [5th, 10th, 20th, 50th] largest runoff event lines
  ...
  for $i (0..$#selected_ranks) {
    ...
    ($interrill_det,$avg_det,$max_det,$det_pt,
     $avg_dep,$max_dep,$dep_pt,$sed_del,$enrich) = split ' ', $rest; 
...
print TEMP "<p>
       <table border=1 cellpadding=12>
        <tr><th bgcolor='#ffddff'>Rank
         <th bgcolor='#ffddff'>Runoff<br>(mm)
         <th bgcolor='#ffddff'>Sediment<br>delivery<br>(kg/m)
         <th bgcolor='#ffddff'>Precipitation<br>(mm)
         <th bgcolor='#ffddff'>Date
        </tr>
        <tr><th bgcolor='#ff00ff'>1</th>
         <td align=right>@run_off[$indx[0]]</td>
         <td align=right>$max_sed_del</td>
         <td align=right>$max_precip</td>
         <td>@monthnames[$max_month] $max_day, $max_year</td></tr>
";
   ...
   @color[3]="'#ffaaff'"; @color[4]="'#ffccff'"; @color[5]="'#ffddff'";
      ...
      for $i (0..$#selected_ranks) {
      print TEMP '<tr><th bgcolor=',@color[$i],'>',@selected_ranks[$i] + 1,' (',100/(@selected_ranks[$i] + 1),"- year)</td>
             <td align=right>@run_off[$indx[@selected_ranks[$i]]]</td>
             <td align=right>@sed_delivery[$i]</td>
             <td align=right>@precip[$i]</td>
             <td align=right>@monthnames[@month[$i]]  @day[$i], @selected_year[$i]</td></tr>
";
    }
    ...
    ($interrill_det,$avg_det,$max_det,$det_pt,
     $avg_dep,$max_dep,$dep_pt,$min_sed_del,$enrich) = split ' ', $rest; 
      ...
      print TEMP
"<tr><th bgcolor='#ffddff'>100</td>
       <td align=right>@run_off[$indx[$years]]</td>
       <td align=right>$min_sed_del</td>
       <td align=right>$min_precip</td>
       <td>@monthnames[$min_month] $min_day, $min_year</td></tr>
</table>";
...
#         system @args;						# TEMP DEH
 ...
 <table cellpadding="2" bgcolor="ivory">
  <tr><th colspan=5><font size=+1>SEDIMENT  DELIVERY (kg m<sup>-1</sup>)</font></th><th>Spatial (prob)</th></tr>',"\n";

  for $c (0..$#selected_year) {
    print TEMP '  <tr><th align="left" bgcolor="gold" colspan="5">',@day[$c],' ',@monthnames[@month[$c]],' year ',@selected_year[$c],' (',@probClimate[$c]*100,'%)</th><td></td></tr>',"\n";
#DEH 8
#    for $sn (0..7) {
    for $sn (0..3) {
      print TEMP "<tr>\n";
      for $k (0..4) {
        ...
        print TEMP '<td align="right">',@sed_yields[$sp],'</td>';
        $sp += 1;
      }
      print TEMP '<td bgcolor="gold">',@severe[$sn],' (', @probspatial[$sn]*100, "%)</td></tr>\n";
    }
#    print TEMP "\n";
  }
  print TEMP '  <tr><td>conduct 0</td><td>conduct 1</td><td>conduct 2</td><td>conduct 3</td><td>conduct 4</td><td></td></tr>
  <tr><td bgcolor="gold">(',$probSoil0[0]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil0[1]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil0[2]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil0[3]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil0[4]*100,'%)</td>
   <td>year 0</td></tr>
  <tr><td bgcolor="gold">(',$probSoil1[0]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil1[1]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil1[2]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil1[3]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil1[4]*100,'%)</td>
   <td>year 1</td></tr>
  <tr><td bgcolor="gold">(',$probSoil2[0]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil2[1]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil2[2]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil2[3]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil2[4]*100,'%)</td>
   <td>year 2</td></tr>
  <tr><td bgcolor="gold">(',$probSoil3[0]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil3[1]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil3[2]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil3[3]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil3[4]*100,'%)</td>
   <td>year 3</td></tr>
  <tr><td bgcolor="gold">(',$probSoil4[0]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil4[1]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil4[2]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil4[3]*100,'%)</td>
   <td bgcolor="gold">(',$probSoil4[4]*100,'%)</td>
   <td>year 4</td></tr>
 </table>
';
...
print'   </HEAD>
   ...
   <blockquote>
   <CENTER>
     <hr width=50%><H2>ERMiT Results</H2>
';
   print "<h3 align=center>$clim_name</h3><hr width=50%>";
   print '
   </CENTER>
   ';
   print "\n Average gradient $avg_slope %<br>";
   print "\n Toe gradient     $toe_slope %<br>";
   print "\n Length           $hillslope_length $units<br>";
   print "\n Severity class   '$severityclass'<br>";
   print "\n Soil texture     '$soil_texture'<br>";
   print "\n Vegetation type  '$vegtype'<br>";
   print "\n Rock fragment    $rfg %<br>";
...
<form name="doit">
 <script>
  ...
  document.writeln(' <input type="text" name="sediment0" onChange="javascript:sedchange(0)" value="'+seds+'" size="8"> kg m<sup>-1</sup>')
  document.writeln(' in year 0 following the fire.<br>')
  seds = rounder(whatseds (0.5, cp1),2)
  document.writeln(' There is a ')
  document.writeln(' <input type="text" name="probability1" onChange="javascript:probchange(1)" value="50" size="8">')
  document.writeln(' % chance that sediment yield will exceed ')
  document.writeln(' <input type="text" name="sediment1" onChange="javascript:sedchange(1)" value="'+seds+'" size="8"> kg m<sup>-1</sup>')
  document.writeln(' in year 1 following the fire.<br>')
  seds = rounder(whatseds (0.5, cp2),2)
  document.writeln(' There is a ')
  document.writeln(' <input type="text" name="probability2" onChange="javascript:probchange(2)" value="50" size="8">')
  document.writeln(' % chance that sediment yield will exceed ')
  document.writeln(' <input type="text" name="sediment2" onChange="javascript:sedchange(2)" value="'+seds+'" size="8"> kg m<sup>-1</sup>')
  document.writeln(' in year 2 following the fire.<br>')
  seds = rounder(whatseds (0.5, cp3),2)
  document.writeln(' There is a ')
  document.writeln(' <input type="text" name="probability3" onChange="javascript:probchange(3)" value="50" size="8">')
  document.writeln(' % chance that sediment yield will exceed ')
  document.writeln(' <input type="text" name="sediment3" onChange="javascript:sedchange(3)" value="'+seds+'" size="8"> kg m<sup>-1</sup>')
  document.writeln(' in year 3 following the fire.<br>')
  seds = rounder(whatseds (0.5, cp4),2)
  document.writeln(' There is a ')
  document.writeln(' <input type="text" name="probability4" onChange="javascript:probchange(4)" value="50" size="8">')
  document.writeln(' % chance that sediment yield will exceed ')
  document.writeln(' <input type="text" name="sediment4" onChange="javascript:sedchange(4)" value="'+seds+'" size="8"> kg m<sup>-1</sup>')
  document.writeln(' in year 4 following the fire.<br>')
 </script>
<p>
<a href="javascript:percentages(1)">2</a> --
<a href="javascript:percentages(5)">5</a> --
<a href="javascript:percentages(10)">10</a> --
<a href="javascript:percentages(20)">20</a> --
<a href="javascript:percentages(50)">50</a> %
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="javascript:sediments(0)">0</a> --
<a href="javascript:sediments(10)">10</a> --
<a href="javascript:sediments(20)">20</a> --
<a href="javascript:sediments(50)">50</a> --
<a href="javascript:sediments(120)">120</a> kg m<sup>-1</sup>
<br>
<a href="javascript:percentages(2)">50</a> --
<a href="javascript:percentages(5)">20</a> --
<a href="javascript:percentages(10)">10</a> --
<a href="javascript:percentages(20)">5</a> --
<a href="javascript:percentages(50)">2</a> year
</form>
<p>
<!--<img src="gostone.gif">-->
<br>
<!--  <a href="javascript:showtable()">show me the table</a> -->
<FORM><INPUT TYPE="button" VALUE="Show data table" onClick="showtable()"></FORM>
</center>
</body>
</html>
EOP2
...
"
  ...
  if (lc($s) eq "lhh" || lc($s) eq "hll") {			### abb.slp ###
    my $length1 = $hillslope_length_m/3;
    my $length2 = $hillslope_lengthm-$length1;
    $result = "97.5
#
# Slope file created for fire severity $s average gradient $avg_slope toe gradient $toe_slope
#
...
// alert ('sedchange')
  ...
  if (year == 0) {
    if (!isNumber (document.doit.sediment0.value)) {document.doit.sediment0.value = default_sed_del}
//   document.doit.probability.value = ''
    if (document.doit.sediment0.value > sed_del[1]) {document.doit.sediment0.value = sed_del[1]}
    if (document.doit.sediment0.value < sed_del[80]) {document.doit.sediment0.value = sed_del[80]}
...
//    if (document.doit.probability0.value > 99.9) {document.doit.probability0.value = 99.9}
    mysed = document.doit.sediment0.value
    prob = whatprob (mysed, cp0)
    document.doit.probability0.value = rounder(prob*100,2)
  }
  if (year == 1) {
    if (!isNumber (document.doit.sediment1.value)) {document.doit.sediment1.value = default_sed_del}
    if (document.doit.sediment1.value > sed_del[1]) {document.doit.sediment1.value = sed_del[1]}
    if (document.doit.sediment1.value < sed_del[80]) {document.doit.sediment1.value = sed_del[80]}
    mysed = document.doit.sediment1.value
    prob = whatprob (mysed, cp1)
    document.doit.probability1.value = rounder(prob*100,2)
  }
  if (year == 2) {
    if (!isNumber (document.doit.sediment2.value)) {document.doit.sediment2.value = default_sed_del}
    if (document.doit.sediment2.value > sed_del[1]) {document.doit.sediment2.value = sed_del[1]}
    if (document.doit.sediment2.value < sed_del[80]) {document.doit.sediment2.value = sed_del[80]}
    mysed = document.doit.sediment2.value
    prob = whatprob (mysed, cp2)
    document.doit.probability2.value = rounder(prob*100,2)
  }
  if (year == 3) {
    if (!isNumber (document.doit.sediment3.value)) {document.doit.sediment3.value = default_sed_del}
//   document.doit.probability3.value = ''
    if (document.doit.sediment3.value > sed_del[1]) {document.doit.sediment3.value = sed_del[1]}
    if (document.doit.sediment3.value < sed_del[80]) {document.doit.sediment3.value = sed_del[80]}
    mysed = document.doit.sediment3.value
    prob = whatprob (mysed, cp3)
    document.doit.probability3.value = rounder(prob*100,2)
  }
  if (year == 4) {
    if (!isNumber (document.doit.sediment4.value)) {document.doit.sediment4.value = default_sed_del}
    if (document.doit.sediment4.value > sed_del[1]) {document.doit.sediment4.value = sed_del[1]}
    if (document.doit.sediment4.value < sed_del[80]) {document.doit.sediment4.value = sed_del[80]}
    mysed = document.doit.sediment4.value
    prob = whatprob (mysed, cp4)
    document.doit.probability4.value = rounder(prob*100,2)
  }

//  alert('sedchange: '+eval('cp'+year))

//  if (year == 0) {prob = whatprob (mysed, cp0)}
//  if (year == 1) {prob = whatprob (mysed, cp1)}
//  if (year == 2) {prob = whatprob (mysed, cp2)}
//  if (year == 3) {prob = whatprob (mysed, cp3)}
//  if (year == 4) {prob = whatprob (mysed, cp4)}
...
function showtable () {
  ...
  newin.document.writeln(' <tr><th rowspan=2 bgcolor=yellow>sediment delivery<br>kg m<sup>-1</sup><\/th>')
//  newin.document.writeln('     <th colspan=5 bgcolor=yellow>Probability(x>X)<\/th><\/tr>')
    newin.document.writeln('     <th colspan=5 bgcolor=yellow>Percent chance that sediment delivery will be exceeded<\/th><\/tr>')
  newin.document.write('<tr><th bgcolor=yellow>year 0<\/th>')
  newin.document.write('<th bgcolor=yellow>year 1<\/th>')
  newin.document.write('<th bgcolor=yellow>year 2<\/th>')
  newin.document.write('<th bgcolor=yellow>year 3<\/th>')
  newin.document.writeln('<th bgcolor=yellow>year 4</tr>')
  for (var i=1; i<= sed_del.length; i++) {
   newin.document.write(' <tr><td align=right>' + sed_del[i]+'<\/td>')
...
# USDA FS RMRS Moscow FSL coding by david numbers by bill
...
#  $rfg = 20;		# percentage of rock fragments (by volume) in the layer (%)
  ...
  if ($SoilType eq 'sand' || $SoilType eq 'silt' ||
      $SoilType eq 'clay' || $SoilType eq 'loam') {  }
  else {return}
...
#              ki      kr  tauc/shcrit  Ksat/avke
  ...
  $results="95.1
#  WEPP '$soil_texture' '$s$k' soil input file for ERMiT
