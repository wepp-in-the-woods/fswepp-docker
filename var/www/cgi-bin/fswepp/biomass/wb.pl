...
#####  Read user input parameters  #####
   ...
   $harvest_cycle =$parameters{'harvest_cycle'}+0;	# 2004.09.16
...
# initialization for inputs array 
   @runheadd=('Undisturbed forest','Ground harvest','Prescribed burn','Wildfire','Cable harvest','Higher Rx fire','Lower Rx fire','Moderate wildfire','Low wildfire');
...
#  if ($host eq "") {$host = 'unknown';}
   $unique='wepp-' . $$;
#  if ($debug > 1) {print 'Unique? filename= ',$unique,"\n<BR>"}
...
#    if ($debug > 1) {print "Creating Climate File<br>\n"}
     &CreateCligenFile;
...
#       print "<p>the value of ofe1_length is $ofe1_length";#elena
     ...
     if ($debug > 1) {print "Creating Slope File<br>\n"}
     &CreateSlopeFile;
...
#      @args = ("../wepp <$responseFile >$stoutFile 2>$sterFile")
       @args = ("wine ../wepp2010.100.exe <$responseFile >$stoutFile 2>$sterFile");    # DEH 2015.05.06
     }
     system @args;
...
#  $wildfire  = sprintf '%.1f',($out_asypa[3]*640)/$wildfire_cycle;
   ...
   $withbperofnotb = sprintf('%.1f',((100*$witbackground)/$notbackground));
...
##
   ...
   $thin_over_back = sprintf '%.0f', 100 * ($harvest) / $undiwild if ($undiwild>0.001);
...
#  Insert C [line 1556]		Line 1556: Insert the following.
   ...
   $no_thin_low1_over_back = sprintf '%.0f', 100 * ($no_thin_low1-$undiwild) / $undiwild if ($undiwild>0.001);
   $no_thin_high1_over_back = sprintf '%.0f', 100 * ($no_thin_high1-$undiwild) / $undiwild if ($undiwild>0.001);
##
...
#	</font>
     if ($soil eq 'sandy loam') {
      ...
      print "
        ...
        <font color=green>
";
     printf '%.0f', 100*($no_thin_high1-$no_road_high) / $no_road_high if ($no_road_high > 0.01);
     print "
         to
";
     printf '%.0f', 100*($no_thin_low1-$no_road_low) / $no_road_low if ($no_road_low > 0.01);
     print "
       </font>
";
    }	# if ($soil eq 'sandy loam') {
...
#    $tempmax=$wildmodraw_over_wildfire_cycle + $thinbackno + $prescribe + $tr_road_max;
     ...
     if ($soil eq 'sandy loam'){
      print "
       ...
       <font color=green>
";
    $tempmin=$wildmodraw_over_wildfire_cycle + $thinbackno + $prescribe + $no_road_min;
    $tempmax=$wildmodraw_over_wildfire_cycle + $thinbackno + $prescribe + $no_road_max;
}
     else{
      print "
       <font color=green>
...
# 2008.06.04 DEH end
     ...
     open WBLOG, ">>../working/_$thisyear/wb.log";	# 2012.12.31 DEH
       ...
       print WBLOG $ampm[$ampmi],"  ",$days[$wday]," ",$months[$mon]," ",$mday,", ",$year+1900, "\"\t";
...
#   $beinp = 0.0 #elena
   while (<PS2>) {
    if (/beinp/) {
       ...
       if ($debug > 2) {print "</pre><b>wps2:</b><br>
                           pcover: $pcover<br>
                           beinp: $beinp<br><pre> $_<br>\n"}
    }
    print MANFILE $_;
...
# end swup
   ...
   if ($debug > 1) {print "[CreateCligenFile]<br>
Arguments:    $args<br>
...
####
    if ($debug > 0) {print "[CreateCligenFilewr]<br>
      Arguments:    $args<br>
      ClimatePar:   $climatePar<br>
      ClimateFile:  $climateFile<br>
      OutputFile:   $outfile<br>
