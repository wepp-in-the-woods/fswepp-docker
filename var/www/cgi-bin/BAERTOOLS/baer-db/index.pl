...
###############################################################################
 ...
 # the date range from the user.
    sub PrintDatesInputLine {
        ...
        # -CMA 2014.02.03.
        my $c_CommonAttrs = "style='width:7em'
    ...
    sub PrintLocationsSelectDiv {
        ...
        foreach my $param_name ( keys %param_indices ) {
            ...
            if ( ref($param) eq 'ARRAY' ) {
                ...
                my @valid_values =
                  grep { defined($_) && /^[-+]?\d+(\.\d+)?$/ } @$param;
                if (@valid_values) {
                    $rPageState->[$idx] =
                      $valid_values[0];    # Keep the first valid value
                }
                else {
                    $rPageState->[$idx] = '';
                }
        ...
        foreach my $param_name ( keys %param_indices ) {
            ...
            if ( ref($param) eq 'ARRAY' ) {
                ...
                # If it's an array, validate each element
                my @valid_values =
                  grep { defined($_) && is_valid_date($_) } @$param;
                if (@valid_values) {
                    $rPageState->[$idx] =
                      $valid_values[0];    # Keep the first valid value
                }
...
# us to filter on.
    sub PrintLocationsSelectElem {
      ...
      # it submits, depends on the location-type URL param.
        print(
            "<select name='$LocType' size='10' multiple='multiple'
														style='width:15em'>\n"
        );
...
###############################################################################
 ...
 # called in map view.
    ...
    # Javascript below so that the Javascript code can access it.
        ...
        # console.log("rPoints: ", rPoints);
        # print STDERR 'rPoints: ', rPoints;

        # Tested by Test140206_1158.  -CMA 2014.02.07.

        # Google Maps to Leaflet Conversion (Part 2) -Intern: L.W. 2021.06.07
        print( "
	...
	var c_map_type    = new L.TileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png');

	// Show the Leaflet map in our <divMap> div.
	var divMap = 'divMap';
	var map = new L.Map( divMap, { center: new L.LatLng($MapLat, $MapLon), zoom: $Zoom, layers: c_map_type});

	var /*MarkerData[]*/ c_arrMarkerData = $MarkerData;
	var markerClusters = L.markerClusterGroup();
	
	console.log('length: ', c_arrMarkerData.length); 
	for( var i = 0; i < c_arrMarkerData.length; i++ )
	{
		...
		// This is tho show the fires names while hovering over the markers 
		// (Otherwise it would have required a mouse click)
...
<link rel='stylesheet' type='text/css' href='/BAERTOOLS/baer-db/baer-db.css' />
	" );
        ...
        if ($bMapView) {     # -CMA 2013.11.26.
                             # Include Leaflet API.
             # Google Maps to Leaflet Conversion (Part 1) -Intern: L.W. 2021.06.07
            ...
            print( "
				...
				<link rel='stylesheet' href='/leaflet_cluster/screen.css' />

				<link rel='stylesheet' href='/leaflet_cluster/dist/MarkerCluster.css' />
				<link rel='stylesheet' href='/leaflet_cluster/dist/MarkerCluster.Default.css' />
				<script src='/leaflet_cluster/dist/leaflet.markercluster-src.js'></script>
		" );
        }
...
###############################################################################
 ...
 # value is found in a given list of (previously selected) values.
    ...
    # submitted value.
    sub PrintPreselOption {
        my ( $DispVal, $raSelVals, $SubmitValParam ) = @_;
...
###############################################################################
	...
	sub PrintSelectedItemsLine {
		my ( $Label, $raSel ) = @_;
		use HTML::Entities;

		print("<span class='SpanIndent'></span><b>", encode_entities($Label), ":</b>\n");
		...
		if ( !@$raSel ) {
			...
			for (@$raSel) {
				push( @ListToPrint, encode_entities($_) );
			}
		}
...
<form method='get'>
	...
	" );
        ...
        my @HiddenParams =
          BwdPageState::HiddenParams( TheBwdPageState::GetC() );
