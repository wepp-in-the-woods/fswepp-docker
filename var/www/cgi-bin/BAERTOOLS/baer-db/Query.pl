...
# BaerProj SUBS ################################################################
	...
	sub AddTreatmentFromMdb { my( $this, $rTmtFromMdb ) = @_;
...
# -CMA 2013.08.23.
sub NewFromMdb { my( $rProjFromXml ) = @_;
	my @This;
...
sub OwnsTreatmentFromMdb { my( $this, $rTmtFromMdb ) = @_;
...
sub ClearMapProjects { my( $this ) = @_;
	$this->[ 16 ] = [];
...
sub Filter { my( $this, $raProjsParam ) = @_;
	# Filter by requested forests.
	my $raProjsRet =
			_rFilterOnList(
					$raProjsParam, $this->[ 0 ], \&BaerProj::MatchesReqForest );
	...
	$raProjsRet = _rFilterOnList( $raProjsRet, $this->[ 9 ], $fnMatchesState );
	...
	$raProjsRet =
			_rFilterOnList(
						$raProjsRet, $this->[ 1 ], \&BaerProj::MatchesReqTmt );
...
sub NewExpandedRangeItr { my( $this, $Start, $End ) = @_;
	return OnRangeItr::NewFromExceptions(
								$Start, $End, $this->[ 11 ], $this->[ 12 ] );
...
sub SetOffsetSize { my( $this, $iOffset, $bExpand ) = @_;
...
sub SetPageStart { my( $this, $PageStart ) = @_;
	$this->[ 5 ] = $PageStart;
...
sub SetShowTotals{ my( $this, $bShow ) = @_;
	$this->[ 7 ] = $bShow;
...
sub SetSortType { my( $this, $SortType ) = @_;
	$this->[ 4 ] = $SortType;
...
sub _rFilterOnDate { my( $raProjs, $DateParam, $bEnd ) = @_;
	...
	for( @$raProjs ) {
		...
		my $bReject = $bEnd ? $ProjDate gt $FiltDate : $ProjDate lt $FiltDate;
...
sub _rFilterOnList { my( $raProjs, $raFilter, $rfnTest ) = @_;
...
sub _IsParamEq { my( $ParamName, $CmpVal ) = @_;
...
sub _rSortByDist { my( $this, $raProjs ) = @_;
...
sub GetRelink { my( $rPageState, $LinkText ) = @_;
...
# params.
sub GetRelinkUrl { my( $rPageState ) = @_;
...
sub GetSortPhrase {
	...
	if( "cost" eq $SortType ) {
		...
		my $Dms =
				"(" . BwdPageState::DegN( $rPageState ) . "&deg; N, " .
				BwdPageState::DegW( $rPageState ) . "&deg; W)";
...
sub GetTotalsRelink {
	...
	return GetRelink( $rLinkPageState, $bShowTotals ? "hide" : "show totals" );
...
{	package OnRangeItr;
	...
	sub NewFromExceptions { my( $Start, $End, $bDefOn, $raExcOffsets ) = @_;
		my @This;
...
sub Load {
	...
	@$_raProjs =
			sort { BaerProj::FireStrt( $b ) cmp BaerProj::FireStrt( $a ) }
																	@$_raProjs;
...
sub NewFromMdb { my( $rTmtFromXml ) = @_;
	my @NewObj;
