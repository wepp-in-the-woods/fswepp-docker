...
if (oldrow != undefined) { document.getElementById("r"+oldrow).style.backgroundColor="blue" }
if (oldcol != undefined) { document.getElementById("c"+oldcol).style.backgroundColor="blue" }
     what = "r"+row;
...
if (oldrow != undefined) { document.getElementById("pr"+oldrow).style.backgroundColor="blue" }
if (oldcol != undefined) { document.getElementById("pc"+oldcol).style.backgroundColor="blue" }
     what = "pr"+row;
     document.getElementById(what).style.backgroundColor="#ff0000"
     oldrow = row;
     what = "pc"+col;
...
//   alert (myLink)
//     document.ELEVdepGraphNS.src=myLink
     document.getElementById("bin_count").innerHTML=val+" cells with deposition between "+lower+" and "+upper+" "+depunit;
   }
...
#         print "     <td width=6 height=5 bgcolor='$color' onClick='DepoClick($row,$col,$val,\"$depunit\",\"$unique\")' onMouseover=\"document.getElementById('deprcv').innerHTML='[$row, $col]: $val $depunit'\"></td>\n";
          print "     <td width=6 height=5 bgcolor='$color' onClick='DepoClick($row,$col,$val,\"$depunit\",\"$unique\")' onMouseover=\"document.getElementById('deprcv').innerHTML='$val $depunit deposition at row $row col $col'\"></td>\n";
        }
        else {
          print "     <td width=6 height=5 bgcolor='$color'></td>\n";}
      }			# for ($col=0...)
      print "     <td width=4 bgcolor=blue id=\"r$row\"></td>\n";	# DEH 110525
...
';
      for ($col=0;$col<$num_cols;$col++) {
        print "     <td width=5 height=4 bgcolor=blue id=\"c$col\"></td>\n";
      }
      print '    </tr>
   </table>
...
$depth = $max_r / $bulkDensity / 100;
   ...
   print "    <font size=-1>
   <span id='deprcv'>[row, col]: deposition</span><br>
   Min: $min_r<br>
   Max: $max_r<br>
   Range: $range<br>
   Mean: @meandep[1]<br>
   ...
   Maximum deposition at row $max_dep_row column $max_dep_col<br>
   Maximum deposition depth: $depth m over 10 x 10 m cell<br>
";
if ($range > $min_range) {
   ...
   <span id='bin_count'>[click on histogram axis below for histogram information]</span><br><br>
...
#      print "     <td height='5' bgcolor='$color' onClick=\"document.deposition.depos_hist.value='[$real &mdash; $realh $depunit]: @histo[$his]'\">
       print "     <td height='10' bgcolor='$color' onClick='javascript:DepoHistClick($real,$realh,@histo[$his],\"$depunit\")'></td>\n";
     }		# for ($his=0...)
...
#      $max_r1=-999999;						# DEH ?
      ...
      for ($row=0;$row<$num_rows-1;$row++) {
        ...
        @cell=split ' ',$_;
        for ($col=0;$col<$num_cols;$col++) {
#	  @cell[$col] = 0 if (@cell[$col] < 0);			# DEH
#         $min_r = @cell[$col] if (@cell[$col] < $min_r);	# DEH
          if (@cell[$col] < $min_r) {
            $min_r = @cell[$col];
            $min_prod_row = $row;
            $min_prod_col = $row;
...
#         print "     <td width=6 height=5 bgcolor='$color' onClick=\"document.production.prod.value='[$row, $col]: $val $produnit'\"></td>\n";
          print "     <td width=6 height=5 bgcolor='$color' onClick='ProdClick($row,$col,$val,\"$depunit\",\"$unique\")' onMouseover=\"document.getElementById('prodrcv').innerHTML='$val $depunit production at row $row col $col'\"></td>\n";
        }
        else {
          print "     <td width=6 height=5 bgcolor='$color'></td>\n";
        }
      }			# for ($col=0...)
      print "     <td width=4 bgcolor=blue id=\"pr$row\"></td>\n";	# DEH 110525
...
';
      for ($col=0;$col<$num_cols;$col++) {
        print "     <td width=5 height=4 bgcolor=blue id=\"pc$col\"></td>\n";
      }
      print '    </tr>
   </table>
...
$prod_depth = $max_r / $bulkDensity / 100;
   ...
   print "    <font size=-1>
   <span id='prodrcv'>[row, col]: production</span><br>
   Min: $min_r<br>
   Max: $max_r<br>
   Range: $range<br>
   Mean: @meanprod[1]<br>
   ...
   Maximum production at row $max_prod_row column $max_prod_col<br>
...
#                           <input type='text' name='prod_hist' readonly> [production range]: count &mdash; click on histogram axis below</form>
   print "   <form name='production'><input type='text' name='prod' readonly> [row, col]: production &mdash; click on production grid above<br>
   <input type='text' name='prod_hist' readonly> [production range]: count &mdash; click on histogram axis below</form>
...
#########################
 ...
 <td>
  <td align=top>
   <img src='/BAERTOOLS/ravel/DEMprodGraphXYS.php?file=$unique&y=$max_dep_row&s=1' name='ELEVprodGraphWE'><br>
   <img src='/BAERTOOLS/ravel/DEMprodGraphXYS.php?file=$unique&x=$max_dep_col&s=1' name='ELEVprodGraphNS'><br>
  </td>
 </tr>
</table>
";
