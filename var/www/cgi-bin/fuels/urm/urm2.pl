...
if ( ( $weedname =~ m/" "/ ) || ( $weedname =~ m/""/ ) || ( not($weedname) ) ) {
    $pspecies = "Plant";
    $wepaflag = 1;
}
if ( ( $species =~ m/" "/ ) || ( $species =~ m/""/ ) || ( not($species) ) ) {
    $pspecies = "Plant";
    $wepaflag = 2;
}

if ( ( ( $weedname =~ m/" "/ ) || ( $weedname =~ m/""/ ) || ( not($weedname) ) )
    && ( ( $species =~ m/" "/ ) || ( $species =~ m/""/ ) || ( not($species) ) )
  )
{
    $pspecies = "Plant";
}
    ...
    if ( $wepaflag == 2 ) {
        ...
        if ( $weedname == 3 ) {
            $pspecies = "Cardaria chalapa/lenspod whitetop";
        }
        if ( $weedname == 4 ) { $pspecies = "Cardaria draba/whitetop" }
        if ( $weedname == 5 ) {
            $pspecies = "Cardaria pubescens/hairy whitetop";
        }

        if ( $weedname == 6 ) {
            $pspecies = "Cardus nutans/nodding plumeless thistle";
        }
        if ( $weedname == 7 ) {
            $pspecies = "Centaurea biebersteinii/spotted knapweed";
        }
        if ( $weedname == 8 ) { $pspecies = "Centaurea diffusa/white knapweed" }
        if ( $weedname == 9 ) {
            $pspecies = "Centaurea solstitialis/yellow star-thistle";
        }
        if ( $weedname == 10 ) {
            $pspecies = "Centaurea triumfettii/squarrose knapweed";
        }
        ...
        if ( $weedname == 17 ) {
            $pspecies = "Hieraceum aurantiacum/orange hawkweed";
        }
        if ( $weedname == 18 ) {
            $pspecies = "Hieracium caespitosum/meadow hawkweed";
        }
        if ( $weedname == 19 ) {
            $pspecies = "Hypericum perforatum/common St. Johnswort";
        }
        if ( $weedname == 20 ) { $pspecies = "Isatis tinctoria/Dyer\"s woad" }
        if ( $weedname == 21 ) {
            $pspecies = "Leucanthemum vulgare/oxeye daisy";
        }
        if ( $weedname == 22 ) {
            $pspecies = "Linaria dalmatica/Dalmatian toadflax";
        }
        if ( $weedname == 23 ) {
            $pspecies = "Linaria vulgaris/butter and eggs";
        }
        if ( $weedname == 24 ) {
            $pspecies = "Onopordum acanthium/Scotch cottonthistle";
        }
        if ( $weedname == 25 ) {
            $pspecies = "Potentilla recta/sulphur cinquefoil";
        }
        if ( $weedname == 26 ) {
            $pspecies = "Salvia aethiopis/Mediterranean sage";
        }
        if ( $weedname == 27 ) {
            $pspecies = "Senecio jacobaca/stinking willie";
        }
        if ( $weedname == 28 ) {
            $pspecies = "Sonchus arvensis/field sowthistle ";
        }
        if ( $weedname == 29 ) { $pspecies = "Sonchus asper/spiny sowthistle" }
        if ( $weedname == 30 ) {
            $pspecies = "Taeniatherum caput-medusae/medusahead";
        }
    }
...
if ( $sitepresence == 1 ) { $psitepresence = "Yes" }
    ...
    $psitepresence = "No";
    $firenote[0] =
      "<p>Plant did not accur on site, can not calculate survivorship";
}

$plifeduration = param('life_duration');
$lifeduration  = lc($plifeduration);
if ( $lifeduration =~ /annual/ ) {
    $firenote[1] = "<p>Plant is an annual, it does not survive year to year";
}
...
if ( ( $refractory == 2 ) && ( $seedbanking == 0 ) ) {
    $seedbanking = 1;
    $pnote =
"for a seed to be sun activeded there must be seed banking,<br>your selection of seed bank no and refractory yes has been changed to seed banking yes and refractory yes";
    $seedbankflag = 1;
    print "<p>$pnote";
}
...
for ( $j = 0 ; $j < $numtreatments ; $j++ ) {
    if ( $treatmentsf[$j] == 1 ) {
        ...
        if    ( $timeofthin == 0 ) { $ptimeofthin = "One Year after Thinning"; }
            ...
            $ptimeofthin = "More Than One Year after Thinning";
        }
...
##@onanimaldis=(2,2,2);
    ...
    @onsitecol =
      ( [@onlongdis], [@onshortdis], [@ongravitydis], [@onlowseeddis] );
...
##@offanimaldis=(1,0.5,0.1);
    ...
    @offsitecol =
      ( [@offlongdis], [@offshortdis], [@offgravitydis], [@offlowseeddis] );
    ...
    if ($indexdebug) {
        print
"<b>seed dispersal is: </b>$pseeddis and <b>sitepresence is: </b>$sitepresence";
        print
"<p><b>oncolindex is: </b> $onsiteindex <p><b>offsiteindex is: </b>$offsiteindex";
    }
...
####clasification of moistercond with index j
    ...
    if ($indexdebug) {
        print
"<b>the moisture condition is :</b>$pmoisturecond ($moisturecond)<p> <b>duffdepth is: </b>$pduffdepth";
    }
...
###################################################
    ...
    @sixduff   = ( 2, 2, 2, 3 );
    @heatpent  = (
        [@halfduff], [@oneduff],  [@twoduff], [@threeduff],
        [@fourduff], [@fiveduff], [@sixduff]
    );
    ...
    if ($indexdebug) {
        print
"<p><b>depth of lethal temp is: </b> $pheatpentindex and  $heatpentindex";
    }
...
######################################################
    ...
    if ( $seedbanking == 2 ) {
        ...
        if ( $seeddis == 4 ) {
            $storedseedindex[0] = 0.1;
            $storedseedindex[1] = 0.1;
        }
        else {
            if ( $seedbanking == 0 ) {
                if ( ( $treatmentcond == 1 ) || ( $treatmentcond == 2 ) ) {
                    $storedseedindex[0] = $noseedbank[0];
                    $pheatpentindexo = "unburned";
                }
                else {
                    $storedseedindex[0] = -99;
                    $thinreproindex = 1;
                ...
                if ( $refractory == 2 ) {
                    if ( ( $treatmentcond == 1 ) || ( $treatmentcond == 2 ) ) {
                        $storedseedindex[0] = $refracseed[0];
                        $pheatpentindexo = "unburned";
                    }
                    else {
                        $storedseedindex[0] = -99;
            ...
            if ($indexdebug) {
                print
"<p><b>the storedseedindex for year zero is: </b> $storedseedindex[0]";
...
####years 1,5,10
            for ( $k = 1 ) {
                if ( $seedbanking == 0 ) {
                    if ( ( $treatmentcond == 3 ) ) {
                        $storedseedindex[$k] = $noseedbank[0];
                        $pheatpentindex = "unburned";
                    }
                    else {
                    ...
                    if ( $refractory == 2 ) {
                        if ( ( $treatmentcond == 3 ) ) {
                            $storedseedindex[$k] = $refracseed[0];
                            $pheatpentindex = "unburned";
                        }
                        else {
                ...
                if ($indexdebug) {
                    print
"<p><b>the storedseedindex for years 1,5,10 is: </b> $storedseedindex[1]";
                }
            }
        }
    }
...
####################################################
    ...
    for ( $k = 1 ) {
        if    ( $treatmentcond == 0 ) { $tminsoil[$k] = $moisturecond; }
        elsif ( ( $treatmentcond == 1 ) || ( $treatmentcond == 2 ) ) {
            $tminsoil[$k] = $moisturecond + $mineralexpo;
        }
        elsif ( $treatmentcond == 3 ) { $tminsoil[$k] = $mineralexpo; }
    }
####################################################
...
####################################################
    ...
    for ( $k = 0 ; $k <= 1 ; $k++ ) {
        ...
        elsif ( ( $tminsoil[$k] > 0 ) && ( $tminsoil[$k] < 25 ) ) {
            $minsoilindex[$k] = 1;
        }
        elsif ( ( $tminsoil[$k] >= 25 ) && ( $tminsoil[$k] < 50 ) ) {
            $minsoilindex[$k] = 2;
        }
        elsif ( ( $tminsoil[$k] >= 50 ) && ( $tminsoil[$k] < 72 ) ) {
            $minsoilindex[$k] = 3;
        }
        ...
        if ($indexdebug) {
            print
"<p><b>total mineral soil for reproduction in year $k is:</b> $tminsoil[$k] ";
...
##########################################################
    ...
    if ( ( $treatmentcond == 1 ) || ( $treatmentcond == 2 ) ) {
        $fcanopycover[0] = $thincanopycover;
    }    #####cn
    else { $fcanopycover[0] = -99; }

    if (   ( ( $fcanopycover[0] < 0 ) || ( $fcanopycover[$k] > 100 ) )
        && ( $fcanopycover[0] != -99 ) )
    {
        print "something is wrong with the canopy percentages";
    }
    if ( ( $fcanopycover[0] >= 0 ) && ( $fcanopycover[0] < 34 ) ) {
        $canopyinte[0] = 0;
        $pcanopyinte = "0-33";
    }
    elsif ( ( $fcanopycover[0] >= 34 ) && ( $fcanopycover[0] < 67 ) ) {
        $canopyinte[0] = 1;
        $pcanopyinte = "34-66";
    }
    elsif ( ( $fcanopycover[0] >= 67 ) && ( $fcanopycover[0] <= 100 ) ) {
        $canopyinte[0] = 2;
        $pcanopyinte = "67-100";
    }
    ...
    for ( $k = 1 ) {
        ...
        { $fcanopycover[$k] = $firecanopy; }    #####cn
            ...
            $fcanopycover[$k] = $firecanopy;
        }                                       #####cn
        else { $fcanopycover[$k] = $thincanopycover; }    #####cn

        if ( ( $fcanopycover[$k] < 0 ) || ( $fcanopycover[$k] > 100 ) ) {
            print "something is wrong with the canopy percentages";
        }
        if ( ( $fcanopycover[$k] >= 0 ) && ( $fcanopycover[$k] < 34 ) ) {
            $canopyinte[$k] = 0;
            $pcanopyinte = "0-33";
        }
        elsif ( ( $fcanopycover[$k] >= 34 ) && ( $fcanopycover[$k] < 67 ) ) {
            $canopyinte[$k] = 1;
            $pcanopyinte = "34-66";
        }
        elsif ( ( $fcanopycover[$k] >= 67 ) && ( $fcanopycover[$k] <= 100 ) ) {
            $canopyinte[$k] = 2;
            $pcanopyinte = "67-100";
        }
...
######print "<p><b>the reproduction canopy cover index is:</b>$canopycoverindex[$k]";#####cn
        if ($indexdebug) {
            ...
            print
"<p><b>the reproduction canopy cover index is:</b>$canopycoverindex[$k]";
        }
    }
...
##$nutrients=1;
        ...
        @frlbulb    = ( 0,  0,   0,   0,   -1,  -2 );
        @suseedsurvship =
          ( [@frlduff], [@frlstolon], [@frlcrown], [@frlrhizone], [@frlbulb] );
        if ( $rootloc =~ m/duff/ ) {
            $survivorFindex = $suseedsurvship[0][ $heatpentindex - 1 ];
        }
        elsif ( $rootloc =~ m/stolon/ ) {
            $survivorFindex = $suseedsurvship[1][ $heatpentindex - 1 ];
        }
        elsif ( $rootloc =~ m/crown/ ) {
            $survivorFindex = $suseedsurvship[2][ $heatpentindex - 1 ];
        }
        elsif ( $rootloc =~ m/rhizome/ ) {
            $survivorFindex = $suseedsurvship[3][ $heatpentindex - 1 ];
        }
        elsif ( $rootloc =~ m/bulb/ ) {
            $survivorFindex = $suseedsurvship[4][ $heatpentindex - 1 ];
        }
    }
...
##$nutrients=0;
        ...
        @suseedsurvship =
          ( [@srlduff], [@srlstolon], [@srlcrown], [@srlrhizone] );
        if ( $rootloc =~ m/duff/ ) {
            $survivorFindex = $suseedsurvship[0][ $heatpentindex - 1 ];
        }
        elsif ( $rootloc =~ m/stolon/ ) {
    ...
    else { print "<p><b>your life form is a tree please used Fvs-FFE</b>"; }

    if ($indexdebug) {
        print
"<p><b>survivorFindex is:</b> $survivorFindex <p><b>lifeform is: </b>$lifeform";
...
##########################################
    ...
    if ( ( $startcanopy >= 0 ) && ( $startcanopy < 33 ) ) {
        $sucanopyinte  = 0;
        $psucanopyinte = "0-33";
    }
    elsif ( ( $startcanopy >= 33 ) && ( $startcanopy < 67 ) ) {
        $sucanopyinte  = 1;
        $psucanopyinte = "33-67";
    }
    elsif ( ( $startcanopy >= 67 ) && ( $startcanopy <= 100 ) ) {
        $sucanopyinte  = 2;
        $psucanopyinte = "67-100";
    }
...
###########calculation of changes
    ...
    for ( $k = 1 ) {
        ...
        { $change[$k] = $startcanopy - $firecanopy; }    #####cn
            ...
            $change[$k] = ( $startcanopy - $firecanopy );
        }                                                #####cn
        else { $change[$k] = $startcanopy - $thincanopycover; }    #####cn
...

    {
        if ( $change[$k] < 10 ) {
            ...
            if ( $sucanopyinte == 0 ) {
                ...
                @sufcanopycover =
                  ( [@suintolerant], [@sumoderate], [@sutolerant] );
                $sucanopycoverindex[$k] =
                  $sufcanopycover[$shadetol][ $sufcanopyinte[$k] ];
            }
        ...
        if (   ( $shadetol == 1 )
            && ( abs( 100 - ( $startcanopy + $fcanopycover[$k] ) ) <= 10 ) )
        {
            $sucanopycoverindex[$k] = 1;
        }
        if ($indexdebug) {
            print
"<p><b>sucanopycoverindex in year $k is: </b> $sucanopycoverindex[$k]";
            print
"<p><b>survivorship final canopy cover is: </b>$fcanopycover[$k] <b>the change is:</b>$change[$k]";
            print
"<p><b>the category due to canopy change is:</b>$pcanopyduefire <b>start canopy cover interval is:</b>$psucanopyinte";
            print "<p><b>the shade tolerance is:</b>$pshadetol";
        }

        if ( $sucanopycoverindex[$k] == 0 ) {
            $psucanopycoverindex[$k] = "No Change";
        }
        elsif ( $sucanopycoverindex[$k] == -1 ) {
            $psucanopycoverindex[$k] = "Small Decrease";
        }
        elsif ( $sucanopycoverindex[$k] == -2 ) {
            $psucanopycoverindex[$k] = "Moderate Decrease";
        }
        elsif ( $sucanopycoverindex[$k] == -3 ) {
            $psucanopycoverindex[$k] = "Large Decrease";
        }
        elsif ( $sucanopycoverindex[$k] == 1 ) {
            $psucanopycoverindex[$k] = "Small Increase";
        }
        elsif ( $sucanopycoverindex[$k] == 2 ) {
            $psucanopycoverindex[$k] = "Moderate Increase";
        }
        elsif ( $sucanopycoverindex[$k] == 3 ) {
            $psucanopycoverindex[$k] = "Large Increase";
        }
...
####################################################
    ...
    if ( ( $mineralexpo >= 0 ) && ( $mineralexpo < 8 ) ) {
        $survivorTindex   = -0.5;
        $psurvivorTindex  = "0-7";
        $ppsurvivorTindex = "Very Low";
    }
    elsif ( ( $mineralexpo >= 8 ) && ( $mineralexpo < 16 ) ) {
        $survivorTindex   = -1;
        $psurvivorTindex  = "8-15";
        $ppsurvivorTindex = "Low";
    }
    elsif ( ( $mineralexpo >= 16 ) && ( $mineralexpo < 24 ) ) {
        $survivorTindex   = -1.5;
        $psurvivorTindex  = "16-24";
        $ppsurvivorTindex = "Low/Moderate";
    }
    elsif ( $mineralexpo >= 24 ) {
        $survivorTindex   = -2;
        $psurvivorTindex  = "bigger than 24";
        $ppsurvivorTindex = "Moderate";
    }

    if ( ( $mineralexpo >= 0 ) && ( $mineralexpo < 25 ) ) {
        $thinsoilindex  = 1;
        $pthinsoilindex = "0-25";
    }
    elsif ( ( $mineralexpo >= 25 ) && ( $mineralexpo < 50 ) ) {
        $thinsoilindex  = 2;
        $pthinsoilindex = "25-50";
    }
    elsif ( ( $mineralexpo >= 50 ) && ( $mineralexpo < 75 ) ) {
        $thinsoilindex  = 3;
        $pthinsoilindex = "50-75";
    }
    elsif ( $mineralexpo >= 75 ) {
        $thinsoilindex  = 4;
        $pthinsoilindex = "bigger than 75";
    }

    if ($indexdebug) {
        print "<p><b>the survivorTindex index is:</b> $survivorTindex";
        print
          "<p><b>the mineral soil exposure from thinning is:</b>$mineralexpo";
...
###############################################################
    ...
    if (   ( $lifeform =~ m/shrub/ )
        ...
        && ( $treatmentcond < 3 ) )
    {
        @shrubdamage = ( -1, -999, -999, -999 );
        for ( $k = 0 ; $k <= 3 ; $k++ ) {
            $shdamageindex[$k] = $shrubdamage[$k];
        }
    }
    else {
        @sdtreat0 = ( -99, -2,  0,   0 );
        ...
        @shrubdamage =
          ( [@sdtreat0], [@sdtreat1], [@sdtreat2], [@sdtreat3], [@sdtreat4] );
...
########################################
    ...
    if ( ( ( $lifeduration =~ m/annual/ ) || ( $lifeduration =~ m/biennial/ ) )
        && ( ( $lifeform =~ m/forb/ ) || ( $lifeform =~ m/grass/ ) ) )
    {
        $maturitycond = 0;
    }
    ...
    for ( $k = 0 ; $k <= 3 ; $k++ ) {
        ...
        if ($indexdebug) {
            print
"<p><b>the maturity time index in year $k is:</b>$maturitytimeindex[$k]";
            print
"<p><b>the maturity TWO index in year $k is:</b>$maturitytwoindex[$k]";
        }
    }
    ...
    if ( $survivorFindex == -99 ) {
        $psurvivorsum = "<p><b>plant is not present</b>";
        $vegrepro     = 0;
        $pvegrepro    = "No";
    }

    if ( ( ( $lifeform =~ m/shrub/ ) && ( $sprouts == 0 ) ) ) {
        $survivorFindex = -99;
    }
    ...
    @survivorsumm =
      ( [@treatsur0], [@treatsur1], [@treatsur2], [@treatsur3], [@treatsur4] );

    for ( $k = 0 ; $k <= 3 ; $k++ ) {
        if ( $lifeform =~ m/shrub/ ) {
            $survivorsum[$k] =
              $survivorsumm[$treatmentcond][$k] + $shdamageindex[$k];
        }
        ...
        if ($indexdebug) {
            ...
            print
"<p>fpcpwpv is: $fpcpwpv equal $survivorFindex+$sucanopycoverindex[1]+$weed+$vegrepro ";
            print
"<p>tpcpwpv is: $tpcpwpv equal $survivorTindex+$sucanopycoverindex[0]+$weed+$vegrepro ";
        }
    }
        ...
        if ( ( $canopycoverindex[1] == 0.1 ) && ( $j != 0 ) ) {
            ...
            if ( $refractory == 2 )    #refractory
            {
                if ( $storedseedindex[1] == 0.1 ) { $reprosum[$j] = 0.01 }
                    ...
                    @reprosumm   = (
                        [@treatrepro0], [@treatrepro1],
                        [@treatrepro2], [@treatrepro3],
                        [@treatrepro4]
                    );

                    $reprosum[$j] =
                      $reprosumm[$treatmentcond][$j] / $maturitytimeindex[$j];
                    if ($indexdebug) {
...
##$onpoffps=$onsiteindex+$offsiteindex+$storedseedindex[1];
                ...
                if ( $survivorsum[$j] >= 0 ) {
                    ...
                    @templist0 = sort( $storedseedindex[0], $adjoncolindex[$j],
                        $offsiteindex );    ####MEV
                    $max0      = pop(@templist0);
                    @templist1 = sort( $storedseedindex[1], $adjoncolindex[$j],
                        $offsiteindex );    ####MEV
                    $max1 = pop(@templist1);
                    ...
                    if ( ( $max1 == 0.1 ) && ( $j != 0 ) ) {
                        ...
                        @bbtreatrepro0 =
                          ( -99, $sponpofpw, $sponpofpw, $sponpofpw );
                        @bbtreatrepro1 = (
                            $sponpofpwzero, $sponpofpw,
                            $sponpofpw,     $sponpofpw
                        );
                        @bbtreatrepro2 = (
                            $sponpofpwzero, $sponpofpw,
                        ...
                        @bbtreprosumm  = (
                            [@bbtreatrepro0], [@bbtreatrepro1],
                            [@bbtreatrepro2], [@bbtreatrepro3],
                            [@bbtreatrepro4]
                        );

                        if ( $lifeduration =~ m/perennial/ ) {
                            $reprosum[$j] = $bbtreprosumm[$treatmentcond][$j] /
                              $maturitytimeindex[$j];
                            if ($treatmentflag) {
                                if ( $weed == 1 ) {
                                    $reprosum4[$j] =
                                ...
                                if ($indexdebug) {
                                    print "<p>the refractory is:$refractory";
                                    print
"<p>the repro sum in year $j is:$reprosum[$j]";
                        ...
                        if ( $lifeduration =~ m/biennial/ ) {
                            ...
                            if ( ( $j == 2 ) ) {
                                $reprosum[$j] =
                                  $reprosum[$j] / 2 * $maturitytimeindex[$j];
                            }
                            if ( ( $j == 3 ) ) {
                                $reprosum[$j] =
                                  $reprosum[$j] / 4 * $maturitytimeindex[$j];
                            }
                            if ($treatmentflag) {
                                $reprosum4[$j] =
                                  $bbtreprosumm[4][$j] * $canopycoverindex[1];
                                if ( $j == 3 ) {
                                    $reprosum4[$j] = $reprosum4[$j] / 2;
                                }
                                if ($indexdebug) {
                                    print "<p>the refractory is:$refractory";
                                    print
"<p>the repro sum in year $j is:$reprosum[$j]";
                        ...
                        if ($indexdebug) {
                            ...
                            print
                              "<p>the repro sum in year $j is:$reprosum[$j]";
                            print
"<p>sponpofpwtmstc is: ($storedseedindex[1]+$onsiteindex+$offsiteindex+$weed)*$minsoilindex[1]*$canopycoverindex[1]";
                    ...
                    if ( abs( $survivorsum[$j] ) <= $onsiteindex ) {
                        ...
                        @templist0 =
                          sort( $storedseedindex[0], $adjoncolindex[$j],
                            $offsiteindex );    ####MEV
                        $max0 = pop(@templist0);
                        @templist1 =
                          sort( $storedseedindex[1], $adjoncolindex[$j],
                            $offsiteindex );    ####MEV
                        $max1 = pop(@templist1);
                        if ( $max0 == 0.1 ) { $reprosum[0] = 0.01 }
                        if ( ( $max1 == 0.1 ) && ( $j != 0 ) ) {
...
####$onpoffpwpss=$onpoffps+$weed+$survivorsum[$j];
                            ...
                            @lesstreatrepro0 = (
                                -99,           $sponpofpwpss,
                                $sponpofpwpss, $sponpofpwpss
                            );
                            @lesstreatrepro1 = (
                                $sponpofpwpsszero, $sponpofpwpss,
                                $sponpofpwpss,     $sponpofpwpss
                            );
                            @lesstreatrepro2 = (
                                $sponpofpwpsszero, $sponpofpwpss,
                            ...
                            @lesstreatrepro4 =
                              ( -99, 0, $onpoffpwpss, $onpoffpwpss );
                            @lessreprosumm = (
                                [@lesstreatrepro0], [@lesstreatrepro1],
                                [@lesstreatrepro2], [@lesstreatrepro3],
                                [@lesstreatrepro4]
                            );

                            if ( $lifeduration =~ m/perennial/ ) {
                                $reprosum[$j] =
                                  $lessreprosumm[$treatmentcond][$j] /
                                  $maturitytimeindex[$j];
                                if ($treatmentflag) {
                                    if ( $weed == 1 ) {
                                        $reprosum4[$j] =
                                    ...
                                    if ($indexdebug) {
                                        print
                                          "<p>the refractory is:$refractory";
                                        print
"<p>the repro sum in year $j is:$reprosum[$j]";
                                        print
                                          "<p>reprosum4[$j] is: $reprosum4[$j]";
                                    }
                                }
                            }
                            if ( $lifeduration =~ m/biennial/ ) {
                                $reprosum[$j] =
                                  ...
                                  $maturitytimeindex[$j];
                                if ( ( $j == 2 ) ) {
                                    $reprosum[$j] = $reprosum[$j] / 2 *
                                      $maturitytimeindex[$j];
                                }
                                if ( ( $j == 3 ) ) {
                                    $reprosum[$j] = $reprosum[$j] / 4 *
                                      $maturitytimeindex[$j];
                                }
                                if ($treatmentflag) {
                                    $reprosum4[$j] =
...
"<p>the repro sum in year $j is:$reprosum[$j]";
                            ...
                            if ($indexdebug) {
                                print
                                  "<p>the treatmencondition is:$treatmentcond";
                                print
"<p>the repro sum in year $j is:$reprosum[$j]";
                                print
"<p>sponpofpwpsstmstc is: $sponpofpw+$survivorsum[$j]*$minsoilindex[1]*$canopycoverindex[1]";
                                print "<p>lifeduration is: $lifeduration";
                            }
                        }    ##endelse0.1cond
                    }    ######endifabslesseqon
...
####$spoffpw=($storedseedindex[1],$offsiteindex1+$weed)*$minsoilindex[1]*$canopycoverindex[1];
                            ...
                            @bigtreatrepro0 =
                              ( -99, $spoffpw, $spoffpw, $spoffpw );
                            @bigtreatrepro1 =
                              ( $spoffpwzero, $spoffpw, $spoffpw, $spoffpw );
                            @bigtreatrepro2 =
                              ( $spoffpwzero, $spoffpw, $spoffpw, $spoffpw );
                            @bigtreatrepro3 =
                              ( -99, $spoffpw, $spoffpw, $spoffpw );
                            @bigtreatrepro4 = (
                                -99,
                                0,
                                ( $max1 + $weed ) * 1.4 * $canopycoverindex[1],
                                ( $max1 + $weed ) * 1.4 * $canopycoverindex[1]
                            );
                            @bigreprosumm = (
                                [@bigtreatrepro0], [@bigtreatrepro1],
                                [@bigtreatrepro2], [@bigtreatrepro3],
                                [@bigtreatrepro4]
                            );

                            if ( $lifeduration =~ m/perennial/ ) {
                                $reprosum[$j] =
                                  $bigreprosumm[$treatmentcond][$j] /
                                  $maturitytimeindex[$j];
########print "<p>reprosum[$j]is:$reprosum[$j]and maturitytimeindex is:$maturitytimeindex[$j]";
                                if ($treatmentflag) {
                                    if ( $weed == 1 ) {
                                        $reprosum4[$j] = $bigreprosumm[4][$j] /
                                          $maturitytimeindex[$j];
                                    }
                                    else {
                                        $reprosum4[$j] =
...
"<p>the repro sum in year $j is:$reprosum[$j]";
                                print
"<p>spoffpw is: $spoffpw and ($storedseedindex[1]+$offsiteindex+$weed)*$minsoilindex[1]*$canopycoverindex[1]";
                                print
"<p>spoffzero $spoffpwzero and ($storedseedindex[0]+$offsiteindex+$weed)*$minsoilindex[0]*$canopycoverindex[0]";
                                print "<p>lifeduration is: $lifeduration";
                            }
...
########################################
    ...
    @surheadingF = (
        "One Year<br> after" . $surhead,
        "Five Years <br>after" . $surhead,
        "Ten Years<br> after" . $surhead
    );
...
##@pnutrients=(No,No,No,No);
    ...
    if ( $treatmentsf[$i] == 1 ) {
        @ppseasonalitym = (
            "No Fire",         $ppseasonality[1],
            $ppseasonality[1], $ppseasonality[1]
        );
    }
    if ( $treatmentsf[$i] == 3 ) {
        @ppseasonalitym = (
            "No Fire",         $ppseasonality[3],
            $ppseasonality[3], $ppseasonality[3]
        );
    }
    if ( $treatmentsf[$i] == 4 ) {
        @ppseasonalitym = (
            "No Fire",         $ppseasonality[4],
            $ppseasonality[4], $ppseasonality[4]
        );
    }
    ...
    if ( $survivorFindex == -99 ) {
        ...
        @pfireinpact = (
            $psurvivorFindex, $psurvivorFindex,
            $psurvivorFindex, $psurvivorFindex
        );
    }

    @ppsucanopycoverindex = (
        $psucanopycoverindex[0], $psucanopycoverindex[1],
        $psucanopycoverindex[1], $psucanopycoverindex[1]
    );    ###EV

    for ( $k = 0 ; $k <= 1 ; $k++ ) {
        if ( $seedbanking == 2 ) { $ppstoredseedindex[$k] = "Unknown" }
        else {
            if ( $storedseedindex[$k] == 0.1 ) {
                $ppstoredseedindex[$k] = "Very, Very Low";
            }
            if ( $storedseedindex[$k] == 0.5 ) {
                $ppstoredseedindex[$k] = "Very Low";
            }
            if ( $storedseedindex[$k] == 1 ) { $ppstoredseedindex[$k] = "Low" }
            if ( $storedseedindex[$k] == 2 ) {
                $ppstoredseedindex[$k] = "Moderate";
            }
            if ( $storedseedindex[$k] == 3 ) { $ppstoredseedindex[$k] = "High" }
        }
    }
    @pstoredseedindex = (
        $ppstoredseedindex[0], $ppstoredseedindex[1],
        $ppstoredseedindex[1], $ppstoredseedindex[1]
    );

    for ( $k = 0 ; $k <= 3 ; $k++ ) {
        if ( $seeddis == 5 ) { $padjoncolindex[$k] = "Unknown" }
        else {
            if ( $adjoncolindex[$k] <= 0.1 ) {
                $padjoncolindex[$k] = "Very, Very Low";
            }
            if ( $adjoncolindex[$k] == 0.5 ) {
                $padjoncolindex[$k] = "Very Low";
            }
    ...
    for ( $k = 0 ; $k <= 1 ; $k++ ) {
        if ( $minsoilindex[$k] == 0.1 ) {
            $ppminsoilindex[$k] = "Very, Very Low";
        }
    ...
    @pminsoilindex = (
        $ppminsoilindex[0], $ppminsoilindex[1],
        $ppminsoilindex[1], $ppminsoilindex[1]
    );

    for ( $k = 0 ; $k <= 1 ; $k++ ) {
        if ( $canopycoverindex[$k] == 0.1 ) {
            $ppcanopycoverindex[$k] = "Very, Very Low";
        }
        if ( $canopycoverindex[$k] == 1 ) { $ppcanopycoverindex[$k] = "Low" }
        if ( $canopycoverindex[$k] == 2 ) {
            $ppcanopycoverindex[$k] = "Moderate";
        }    # DEH 2005.01.28
        if ( $canopycoverindex[$k] == 3 ) { $ppcanopycoverindex[$k] = "High" }
    }
    @pcanopycoverindex = (
        $ppcanopycoverindex[0], $ppcanopycoverindex[1],
        $ppcanopycoverindex[1], $ppcanopycoverindex[1]
    );
...
##$pnutrients[1]=$ppnutrients;
    ...
    for ( $k = 0 ; $k <= 3 ; $k++ ) {
        ...
        if ( ( $shdamageindex[$k] == -999 ) ) {
            $pshdamageindex[$k] = "Very, Very High";
        }
    }

    for ( $k = 0 ; $k <= 3 ; $k++ ) {
        if ( $survivorsum[$k] <= -3.5 ) {
            $psurvivorsum[$k] = "Very Large Decrease";
        }
        if ( ( $survivorsum[$k] > -3.5 ) && ( $survivorsum[$k] <= -2.5 ) ) {
            $psurvivorsum[$k] = "Large Decrease";
        }
        if ( ( $survivorsum[$k] > -2.5 ) && ( $survivorsum[$k] <= -1.5 ) ) {
            $psurvivorsum[$k] = "Moderate Decrease";
        }
        if ( ( $survivorsum[$k] > -1.5 ) && ( $survivorsum[$k] <= -0.5 ) ) {
            $psurvivorsum[$k] = "Small Decrease";
        }
        if ( ( $survivorsum[$k] > -0.5 ) && ( $survivorsum[$k] < 0.5 ) ) {
            $psurvivorsum[$k] = "No Change";
        }
        if ( ( $survivorsum[$k] >= 0.5 ) && ( $survivorsum[$k] < 1.5 ) ) {
            $psurvivorsum[$k] = "Small Increase";
        }
        if ( ( $survivorsum[$k] >= 1.5 ) && ( $survivorsum[$k] < 2.5 ) ) {
            $psurvivorsum[$k] = "Moderate Increase";
        }
        if ( $survivorsum[$k] >= 2.5 ) { $psurvivorsum[$k] = "Large Increase" }
    }

    for ( $k = 0 ; $k <= 3 ; $k++ ) {
        if ( ( $seeddis == 5 ) || ( $seedbanking == 2 ) ) {
            $preprosum[$k] = "Unknown";
        }
        else {
            if ( ( $reprosum[$k] < 1.0 ) ) { $preprosum[$k] = "Very, Very Low" }
            if ( ( $reprosum[$k] >= 1.0 ) && ( $reprosum[$k] < 3.0 ) ) {
                $preprosum[$k] = "Very Low";
            }
            if ( ( $reprosum[$k] >= 3.0 ) && ( $reprosum[$k] < 9 ) ) {
                $preprosum[$k] = "Low";
            }
            if ( ( $reprosum[$k] >= 9 ) && ( $reprosum[$k] < 15 ) ) {
                $preprosum[$k] = "Moderate";
            }
            if ( ( $reprosum[$k] >= 15 ) && ( $reprosum[$k] <= 21 ) ) {
                $preprosum[$k] = "High";
            }
...
end_tres
    ...
    if ( $treatmentcond == 3 ) {
        print "<center><font size=4 >Thinned Area</font></center><br>";
    }
...
end31
        if ( $lifeduration =~ m/annual/ ) {
            ...
            print
"<td colspan=4>The plant is an annual, it does not survive year to year</td>";
            print "		</tr>";
            print "		</table>
			<br>";
        }
            ...
            for ( $k = $n ; $k <= 3 ; $k++ ) {
                print "<td>$ppsucanopycoverindex[$k]</td>";
            }
            print "			</tr>";
...
##print"			</tr>";}
			...
			<tr>
				...
				<Font color=#006009>Relative Change in Biomass <br>(Compared to Pre-Treatment)</Font></th>";
                if ( $n == 0 ) {
                    print "<td>$psurvivorsum[0]</td>";
                    print
"<td colspan=3>Fire top killed shrub, but shrub did not sprout, shrub did not survive</td>";
                }
                else {
                    print
"<td colspan=3>Fire top killed shrub, but shrub did not sprout, shrub did not survive</td>";
...
end31
        print "
			<tr>
				...
				<Font color=#006009>Relative Change in Biomass <br>(Compared to Pre-Treatment)</Font></th>";
        print
"<td colspan=4>The plant did not occur on site, can not calculate survivorship</td>";
        print "		</tr>";
        print "		</table>
			<br>";
    }
...
###survival report begin
    if ( $sitepresence == 0 ) {
        print
"<p>The plant did not accur in the treatment area before fuels management. Therefore, there was no plant survival</p>";
    }
    else {
        if ( $lifeduration =~ m/annual/ ) {
            print
"<p>Annuals die at the end of the growing season; there is no year to year survival.</p>";
        }
        else {
            if ( $treatmentcond == 3 ) {
                print
"<p>Pile burning sterilizes the soil and delays plant establisment. Typically, weeds are the first plants to 		colonize areas that were burn piles.</p>";
            }
            else {
                if (
                    (
                ...
                {
                    print
"<p>The fire top killed the shrub and the plant was unable to sprout. Therefore, the shrub died.</p>";
                }
                else {
                    if (
                        (
                    ...
                    {
                        print
"<p>The model assumes that fire top-kills shrubs and even without fire mortality, it takes more than one growing season for the shrub to recover pre-fire bromass.</p>";
                    }
                    if ( ( $shadetol == 2 ) && ( $sucanopycoverindex[0] < 0 ) )
                    {
                        print
"<p>The plant prefers low light levels. The treatment opened the canopy enough to reduce plant biomass</p>";
                    }
                    if ( ( $shadetol == 0 ) && ( $sucanopycoverindex[0] > 0 ) )
                    {
                        print
"<p>The plant prefers open canopy and high light levels. The treatment opened the canopy which increased plant growth.</p>";
                    }
                    if ( $survivorFindex < -1 ) {
                        print
"<p>The fire was so severe that it caused significant plant mortality. To reduce mortality, burn under less severe conditions (e.g., when fuel moistures are higher or in the dormant season)</p>";
                    }
                    if ( $survivorTindex < -1 ) {
                        print
"<p>The thinning caused significant plant mortality. To reduce mortality, alter the thinning prescripton to reduce soil disturbance (e.g., thin when the ground is frozen or with snow pack, or use helicopter removal)</p>";
                    }
...
end51
    ...
    for ( $k = $n ; $k <= 3 ; $k++ ) {
        print "<td>$pcanopycoverindex[$k]</td>";
    }
    print "			</tr>
			<tr>
				<th BGCOLOR=#99ff00>
...
####print "<b>One year after thinning</b>";
    if ( $refractory == 2 ) {
        print
"<p>The plant has refractory seeds that need fire to stimulate germination.</p>";
    }
    else {
        if ( $canopycoverindex[0] == 0.01 ) {
            print
"<p>The light levels are not sufficient for seed germination and survival.</p>";
        }
        else {
            if ( $max0 < 1 ) {
                print
"<p>There are very few viable seeds available to recolonize the treatment area.</p>";
            }
        }
    }
...
###print "<p>storedseedindex[1] is $storedseedindex[1]</p>";
    if ( $refractory == 2 ) {
        if ( $storedseedindex[1] < 1 ) {
            print
"<p>The plant has seeds that need to be stimulated to germinate by fire. There was either no fire or fire severity 		was too low to cause seed germination</p>";
        }
        if ( $storedseedindex[1] >= 1 ) {
            print
"<p>The plants seeds were stimulated to germinate by the fire</p>";
        }
        if ( $canopycoverindex[1] == 0.01 ) {
            print
"<p>The light levels are not sufficient for seed germnination and survival.</p>";
        }
        else {
            if ( $max1 < 1 ) {
                print
"<p>There are very few viable seeds available to recolonize the treatment area.</p>";
            ...
            if ( $minsoilindex[1] < 1 ) {
                print
"<p>There is a very little mineral soil available as a seed bid for seed germination</p>";
            }
            if ( $canopycoverindex[1] < 1 ) {
                print
"<p>The light levels are not appropriate for seed germination and survival</p>";
            }
        }
    }
    else {
...
###############################444
    if ($treatmentflag) {
        for ( $k = 0 ; $k <= 3 ; $k++ ) {
            if ( ( $seeddis == 5 ) || ( $seedbanking == 2 ) ) {
                $preprosum4[$k] = "Unknown";
            }
            else {
                if ( $reprosum4[$k] == 0 ) {
                    $preprosum4[$k] = "No Reproduction";
                }
                if ( ( $reprosum4[$k] > 0 ) && ( $reprosum4[$k] < 1 ) ) {
                    $preprosum4[$k] = "Very, Very Low";
                }
                if ( ( $reprosum4[$k] >= 1 ) && ( $reprosum4[$k] < 3 ) ) {
                    $preprosum4[$k] = "Very Low";
                }
                if ( ( $reprosum4[$k] >= 3 ) && ( $reprosum4[$k] < 9 ) ) {
                    $preprosum4[$k] = "Low";
                }
                if ( ( $reprosum4[$k] >= 9 ) && ( $reprosum4[$k] < 15 ) ) {
                    $preprosum4[$k] = "Moderate";
                }
                if ( ( $reprosum4[$k] >= 15 ) && ( $reprosum4[$k] <= 21 ) ) {
                    $preprosum4[$k] = "High";
                }
...
######survivorship talbe begining
        if ( $sitepresence == 1 ) {
			...
			<tr>
				...
				<Font color=#006009>Relative Change in Biomass <br>(Compared to Pre-Treatment)</Font></th>";
            print
"<td colspan=3>Fire severity was so high that plants did not survive</td>";
            print "		</tr>";
            print "		</table>
			<br>";
        }
...
####
        ...
        for ( $k = $n + 1 ; $k <= 3 ; $k++ ) {
            print "<td>$pstoredseedindex[$k]</td>";
        }
        print "			</tr>";
        if ( $refractory == 3 ) {
            print "
...
if ($pnotaflag) {
    $pnota =
      "<br>There is not a summary for '<i>" . $feisspecies . "</i>' in FEIS";
...
print URMLOG @ampm[$ampmi], "  ", @days[$wday], " ", @months[$mon], " ", $mday,
  ", ", $year + 1900, "\t";
