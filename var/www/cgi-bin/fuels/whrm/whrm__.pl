...
###################################
##writting files with information
################################
...
$spepath = "species/".$taxgroup."/";	# DEH 2005.08.08
...
if ($debug) {print "action=~/Display/<br>\n"}
while(<rank_file>)
{if($_=~/Rank/)
...
<!-- hide from old browsers...
 ...
 function popuphistory() {
    ...
    popupwindow.document.writeln('  <title>Wildlife Response Model Input version history</title>')
    popupwindow.document.writeln(' </head>')
...
// end of hiding -->
	...
	<table align="center" width="100%" border="0">
		...
		<td><img src="/fuels/whrm/images/grayjay2_Pilliod.jpg" alt="Wildlife Habitat Response Model" align="right">
		</td>
		</tr>
	</table>
	<br>
...
with similar life history characteristics. To view general life history information for your selected species, please use the "View Background Information" button.</p></b>
	<br>
	<center>
	<!-- form method="post" name="wildlifem4" action="https://localhost/Scripts/fuels/whrm/test/whrm2.pl" -->
	<form method="post" name="wildlifem4" action="/cgi-bin/fuels/whrm/whrm2_.pl">
   ...
   <hr>
    <table border=0>
     <tr>
      <td valign="top" bgcolor="lightgoldenrodyellow">
       <font face="tahoma, arial, helvetica, sans serif" size=1>
        The Wild Response Model: <b>W</b>i<b>RM</b>
       </font>
      </td>
      <td valign="top">
     ...
     <tr>
      <td valign="top">
       <font face="tahoma, arial, helvetica, sans serif" size=1>
        Wildlife Response Model input interface v.
...
#	for($i=0; $i<=$tempsize-1;$i++)
#	{if($i<=$tempsize-($counter+1))
#		{	$j=$i;
#			if($indexflag[$i]==0){$coverfeacode[$j]=$tempcover[$i];$coverdir[$j]=$tempcoverdir[$i];}
#			else
#			{$coverfeacode[$j]=$tempcover[$i+$counter];$coverdir[$j]=$tempcoverdir[$i+$counter];}
#		}
#	else{
#			$j=$i-$counter;
#			if($indexflag[$i]==0){$coverfeacode[$j]=$tempcover[$i];$coverdir[$j]=$tempcoverdir[$i];}
...
#			{$coverfeacode[$j]=$tempcover[$i+$counter];$coverdir[$j]=$tempcoverdir[$i+$counter];}
#		}
#	if($debug){print "<br>j is $j and coverfeacode[$j] is $coverfeacode[$j]";}	
#	}
	for($i=0; $i<=$tempsize-1;$i++)
	{
...
for($i=0;$i<$nestrow; $i++)
{
print "arnest[$i]=",'"',$nestfeacode[$i],'"',";\n";
##print "alert('nest '+arnest[$i]);\n";
}
print "var arfhabi=new Array();\n";
for($i=0;$i<$fhabirow; $i++)
{
print "arfhabi[$i]=",'"',$fhabifeacode[$i],'"',";\n";
}
print "var arpreava=new Array();\n";
for($i=0;$i<$preavarow; $i++)
{
print "arpreava[$i]=",'"',$preavafeacode[$i],'"',";\n";
}
print "var arcover=new Array();\n";
for($i=0;$i<$coverrow; $i++)
{
print "arcover[$i]=",'"',$coverfeacode[$i],'"',";\n";
##print "alert('cover '+ arcover[$i]);\n";
}
...
endrank1
    ...
    open (spe_file, $spepath .$spefilename)|| Error('open','file');;
if ($debug) {print "We are now acting<br>\n"}
    while(<spe_file>) {
	if($_=~/Literature/){close (spe_file);}
	if($_=~/Nestsite/)
	{$_=<spe_file>;
