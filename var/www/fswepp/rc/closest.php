...
<body link=green>
  <center>
    <font face="gill, trebuchet, tahoma, arial, sans serif">
      ...
      <?php
      //   $version='2010.12.08';
      $version = '2020.04.14';      # split depricated
      $howmany = 5;
      ...
      function dms_to_dec($in)
      {
        ...
        if ($degat = strpos($in, "o")) {
          ...
          if (
            is_numeric($degrees) && is_numeric($minutes) && is_numeric($seconds)
            ...
            <form name="mylatlon" action="closest.php">
              <center>
                Latitude: <input type="text" value="<?php echo $my_latitude ?>" name="latitude"
                  onmouseover=javascript:latstatus(); return true" onmouseout="window.status=''">&nbsp;&nbsp;
                Longitude: <input type="text" value="<?php echo $my_longitude ?>" name="longitude"
                  onmouseover=javascript:lonstatus(); return true" onmouseout="window.status=''">
                ...
                <br>
                <font size=-1>
                  Enter target latitude and longitude to find <?php echo $howmany ?> geographically closest CLIGEN
                  climate stations.<br>
                  <font size=-2>
                    Enter values in decimal degrees, eg 48.24, or degrees minutes seconds, eg 48o14'35"<br>
                    North America is positive latitude and negative longitude;<br>
                    for example Moscow, ID is at latitude 46o43'56", longitude -117o00'00".
                  </font>
                </font>
              </center>
            </form>
...
//    Stanley, ID: 44.22	-114.93
      ...
      echo("my_latitude: $my_latitude<br>");
      echo("my_longitude: $my_longitude<br>");
      echo("units: $units<br>");
      echo("howmany: $howmany<br>");
      echo(is_numeric($my_latitude));
      echo(is_numeric($my_longitude));

      if (
        is_numeric($my_latitude) && is_numeric($my_longitude)
        && $my_latitude < 180
        && $my_latitude > -180
        && $my_longitude < 180
        && $my_longitude > -180
        ...
        if ($FILEhandle) {
          ...
          while (!feof($FILEhandle)) {
            ...
            list($filepath, $lat, $lon, $station, $extra) = explode("\t", $line, 4);
            if ($filepath != '') {
        ...
        if (array_multisort($dist, $file, $stat)) {
          ...
          for ($j = 0; $j < $howmany; $j++) {
            ...
            list($state, $fname) = explode("/", $file[$j], 2);
