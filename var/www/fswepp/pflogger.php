...
<?php		// http://forums.digitalpoint.com/showthread.php?t=1028597
  ...
  $clear=$_REQUEST["clear"];	// start new log file			  -- TBA
...
//  if (!is_numeric($Fp)) {$Fp="";}
  ...
  if ($myFile = fopen($logfile,"r")) {
    ...
    echo "  <table border=1>\n";
    echo "   <tr>\n";
    echo "    <th title='Estimated peak flow rate'>Estimated peak flow rate<br>q<br>(m<sup>3</sup>/s)</th>\n";
    echo "    <th title='Storm runoff'>Storm runoff<br>Q<br>(mm)</th>\n";
    echo "    <th title='Storm precipitation'>Storm precipitation<br>P<br>(mm)</th>\n";
    echo "    <th title='Watershed area'>Watershed area<br>A<br>(ha)</th>\n";
    echo "    <th title='Watershed flow length'>Watershed flow length<br>L<br>(m)</th>\n";
    echo "    <th title='Avg watershed gradient'>Avg watershed gradient<br>Sg<br>(m/m)</th>\n";
    echo "    <th title='Curve Number'>Curve Number<br>CN<br>()</th>\n";
    echo "    <th title='Time of concentration'>Time of concentration<br>Tc<br>(h)</th>\n";
    echo "    <th title='Ponding adjustment factor'>Ponding adjustment factor<br>Fp<br>()</th>\n";
    echo "    <th>notes</th>\n";
    echo "   </tr>\n";
