...
//  echo $logfile;
  ...
  if ($myFile = fopen($logfile,"r")) {
    ...
    echo "   <tr>\n";
if ($units == "ft") {
    echo "    <th bgcolor='gold' title='Estimated peak flow rate'>Estimated peak flow rate<br>q<br>(ft<sup>3</sup>/s)</th>\n";
    echo "    <th bgcolor='lightgreen' title='Storm runoff'>Storm runoff<br>Q<br>(in)</th>\n";
    echo "    <th bgcolor='lightgreen' title='Storm precipitation'>Storm precipitation<br>P<br>(in)</th>\n";
    echo "    <th bgcolor='lightpink' title='Watershed area'>Watershed area<br>A<br>(ac)</th>\n";
    echo "    <th bgcolor='lightpink' title='Watershed flow length'>Watershed flow length<br>L<br>(ft)</th>\n";
    echo "    <th bgcolor='lightpink' title='Avg watershed gradient'>Avg watershed gradient<br>Sg<br>(ft/ft)</th>\n";
}
else {
...
//    echo "<p> $num fields in line $row: <br /></p>\n";
      ...
      echo "   <tr>";
if ($units == "ft") {
        echo "<td align=right>". round($data[0]*35.31,2) . "</td>";	// q (m3/s -> ft3/s)
        echo "<td align=right>". round($data[1]/25.4,1)  . "</td>";	// Q (mm -> in)
        echo "<td align=right>". round($data[2]/25.4,1)  . "</td>";	// P (mm -> in)
        echo "<td align=right>". round($data[3]*2.471,1) . "</td>";	// A (ha -> ac)
        echo "<td align=right>". round($data[4]*3.281,1) . "</td>";	// L (m -> ft)
        echo "<td align=right>". $data[5] . "</td>";	// Sg (m/m -> ft/ft))
        echo "<td align=right>". $data[6] . "</td>";	// CN ()
        echo "<td align=right>". $data[7] . "</td>";	// Tc (h)
        echo "<td align=right>". $data[8] . "</td>";	// Fp
        echo "<td align=right>". $data[9] . "</td>";	// notes
}
else {
      for ($c=0; $c < $num; $c++) {
    ...
    echo "  </table>\n";
echo "Note: log file saved in metric units. ";
echo "<a href='pfloggeru.php?units=ft'>display as US customary</a> | ";
echo "<a href='pfloggeru.php'>display as metric</a>";
